...
def test_vjp_single_arg():
    ...
    assert np.allclose(gradient.to_numpy(), expected), f"Expected {expected}, got {gradient.to_numpy()}"
...
def test_vjp_multiple_args():
    ...
    assert np.allclose(gradients[0].to_numpy(), expected_x), f"Expected {expected_x}, got {gradients[0].to_numpy()}"
    assert np.allclose(gradients[1].to_numpy(), expected_y), f"Expected {expected_y}, got {gradients[1].to_numpy()}"
    assert np.allclose(gradients[2].to_numpy(), expected_z), f"Expected {expected_z}, got {gradients[2].to_numpy()}"
...
def test_vjp_kwargs_only():
    ...
    assert np.allclose(grad_kwargs['x'].to_numpy(), expected_x), f"Expected {expected_x}, got {grad_kwargs['x'].to_numpy()}"
    assert np.allclose(grad_kwargs['y'].to_numpy(), expected_y), f"Expected {expected_y}, got {grad_kwargs['y'].to_numpy()}"
...
def test_vjp_mixed_args_kwargs():
    ...
    assert np.allclose(grad_args[0].to_numpy(), expected_a), f"Expected {expected_a}, got {grad_args[0].to_numpy()}"
    assert np.allclose(grad_args[1].to_numpy(), expected_b), f"Expected {expected_b}, got {grad_args[1].to_numpy()}"
    assert np.allclose(grad_kwargs['scale'].to_numpy(), expected_scale), f"Expected {expected_scale}, got {grad_kwargs['scale'].to_numpy()}"
    assert np.allclose(grad_kwargs['offset'].to_numpy(), expected_offset), f"Expected {expected_offset}, got {grad_kwargs['offset'].to_numpy()}"
...
def test_vjp_nested_structures():
    ...
    assert np.allclose(gradient['x'].to_numpy(), expected_x), f"Expected {expected_x}, got {gradient['x'].to_numpy()}"
    assert np.allclose(gradient['y'][0].to_numpy(), expected_y0), f"Expected {expected_y0}, got {gradient['y'][0].to_numpy()}"
    assert np.allclose(gradient['y'][1].to_numpy(), expected_y1), f"Expected {expected_y1}, got {gradient['y'][1].to_numpy()}"
...
def test_vjp_list_input():
    ...
    assert np.allclose(gradient[0].to_numpy(), expected), f"Expected {expected}, got {gradient[0].to_numpy()}"
...
def test_vjp_complex_computation():
    ...
    assert np.allclose(gradients[0].to_numpy(), expected_x, rtol=1e-5), f"Expected {expected_x}, got {gradients[0].to_numpy()}"
    assert np.allclose(gradients[1].to_numpy(), expected_y, rtol=1e-5), f"Expected {expected_y}, got {gradients[1].to_numpy()}"
...
def test_vjp_edge_cases():
    ...
    assert np.allclose(gradient.to_numpy(), expected_zero), f"Expected {expected_zero}, got {gradient.to_numpy()}"

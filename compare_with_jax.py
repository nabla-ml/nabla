...
def test_signature_behavior():
    ...
    if JAX_AVAILABLE:
        ...
        result1 = vjp1(1.0)
        print(
            f"Single arg vjp(f, x) returns: {type(result1)} with {len(result1)} elements"
        )
        ...
        result2 = vjp2(1.0)
        print(
            f"Multi arg vjp(f, x, y) returns: {type(result2)} with {len(result2)} elements"
        )
        ...
        result3 = vjp3(1.0)
        print(
            f"List arg vjp(f, [x]) returns: {type(result3)} with {len(result3)} elements"
        )
        ...
        result4 = vjp4(1.0)
        print(
            f"Dict arg vjp(f, {{'x': x}}) returns: {type(result4)} with {len(result4)} elements"
        )
    ...
    print(
        f"Multi arg vjp(f, x, y) returns: {type(result2_nb)} with {len(result2_nb)} elements"
    )

    # List as single argument
    def f3_nb(inputs):
        return inputs[0] * nb.array([2.0])

    _, vjp3_nb = nb.vjp(f3_nb, [nb.array([1.0])])
    result3_nb = vjp3_nb([nb.array([1.0])])
    print(
        f"List arg vjp(f, [x]) returns: {type(result3_nb)} with {len(result3_nb)} elements"
    )
    print(f"Content: {result3_nb[0]} (type: {type(result3_nb[0])})")

    # Dict as single argument
    def f4_nb(data):
        return data["x"] * nb.array([2.0])

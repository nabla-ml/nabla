...
class TestVmapCommunicationStress:
    ...
    def test_vmap_all_gather_hidden(self):
        ...
        np_x = jax.random.normal(jax.random.PRNGKey(42), (batch, hidden), dtype=jnp.float32)
    ...
    def test_vmap_hybrid_dp_tp(self):
        ...
        np_x = jax.random.normal(jax.random.PRNGKey(44), (batch, hidden), dtype=jnp.float32)
        x = tensor_from_jax(np_x)
    ...
    def test_vmap_all_to_all_transpose(self):
        ...
        np_x = jax.random.normal(jax.random.PRNGKey(45), (batch, h1, h2), dtype=jnp.float32)
        x = tensor_from_jax(np_x)
        ...
        assert_allclose(result, np_x)
        assert result.sharding.dim_specs[2].axes == ["tp"] # Sharding moved to last axis of logical output
    ...
    def test_vmap_gather_all_axes_stress(self):
        ...
        np_x = jax.random.normal(jax.random.PRNGKey(46), (batch, h1, h2), dtype=jnp.float32)
        x = tensor_from_jax(np_x)
    ...
    def test_vmap_reduce_scatter_sync(self):
        ...
        np_x = jax.random.normal(jax.random.PRNGKey(47), (batch, hidden), dtype=jnp.float32)
        x = tensor_from_jax(np_x)

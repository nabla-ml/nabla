...
from .common import (
    ...
    jax_transpose_wrapper, jax_squeeze_wrapper, jax_unsqueeze_wrapper, 
    jax_slice_wrapper, jax_pad_inverse_slice
)
...
OPS["swap_axes"] = Operation(
    "swap_axes", "VIEW", nb.swap_axes, jax_transpose_wrapper,
    [
...
OPS["transpose"] = Operation(
    "transpose", "VIEW", nb.swap_axes, jax_transpose_wrapper,
    [
...
OPS["reshape"] = Operation(
    "reshape", "VIEW", nb.reshape, jnp.reshape,
    [
        OpConfig("Flatten", ranks=(3,), params={"shape": (-1,)}),
        OpConfig("Reshape_Split", ranks=(1,), params={"shape": (2, 4)}, primal_shapes=((8,),)),
    ],
    get_reshape_args
)

OPS["squeeze"] = Operation(
    "squeeze", "VIEW", nb.squeeze, jax_squeeze_wrapper,
    [
...
OPS["unsqueeze"] = Operation(
    "unsqueeze", "VIEW", nb.unsqueeze, jax_unsqueeze_wrapper,
    [
...
OPS["broadcast_to"] = Operation(
    "broadcast_to", "VIEW", nb.broadcast_to, jnp.broadcast_to,
    [
        OpConfig("Broadcast_Simple", primal_shapes=((3,),), params={"shape": (2, 3)}),
        OpConfig("Broadcast_RankUp", primal_shapes=((3,),), params={"shape": (2, 2, 3)}),
    ],
    standard_get_args
)

OPS["concatenate"] = Operation(
    "concatenate", "VIEW", nb.concatenate, jnp.concatenate,
    [
...
OPS["moveaxis"] = Operation(
    "moveaxis", "VIEW", nb.moveaxis, jax_moveaxis,
    [
...
def test_view_ops(op_name, config_idx):
    ...
    if op.name == "moveaxis" and config.supports_vmap:
        config = OpConfig(description=config.description, ranks=config.ranks, params=config.params, supports_vmap=False, supports_sharding=config.supports_sharding)

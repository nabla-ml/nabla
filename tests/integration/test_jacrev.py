...
def test_vector_to_scalar_jacobian():
    ...
    assert np.allclose(jac_nb.to_numpy(), jac_jax, rtol=1e-5, atol=1e-6), "Jacobian must match"
    assert np.allclose(hess_nb.to_numpy(), hess_jax, rtol=1e-4, atol=1e-5), "Hessian must match"

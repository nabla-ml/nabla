...
def test_single_input_single_output():
    ...
    # Check structure
    print(f"  Nabla VJP structure: {type(grads_nb)}, len={len(grads_nb) if hasattr(grads_nb, '__len__') else 'N/A'}")
    print(f"  JAX VJP structure: {type(grads_jax)}, len={len(grads_jax) if hasattr(grads_jax, '__len__') else 'N/A'}")
    
    # Both should return tuples with one element
    assert isinstance(grads_nb, tuple), f"Nabla should return tuple, got {type(grads_nb)}"
    assert isinstance(grads_jax, tuple), f"JAX should return tuple, got {type(grads_jax)}"
    assert len(grads_nb) == 1, f"Nabla should return tuple with 1 element, got {len(grads_nb)}"
    assert len(grads_jax) == 1, f"JAX should return tuple with 1 element, got {len(grads_jax)}"
    
    # Check values
    assert np.allclose(grads_nb[0].to_numpy(), grads_jax[0]), "Gradient values should match"
...
def test_multiple_inputs_single_output():
    ...
    assert isinstance(grads_nb, tuple), f"Nabla should return tuple, got {type(grads_nb)}"
    assert isinstance(grads_jax, tuple), f"JAX should return tuple, got {type(grads_jax)}"
    assert len(grads_nb) == 2, f"Nabla should return tuple with 2 elements, got {len(grads_nb)}"
    assert len(grads_jax) == 2, f"JAX should return tuple with 2 elements, got {len(grads_jax)}"
    
    # Check values
    assert np.allclose(grads_nb[0].to_numpy(), grads_jax[0]), "First gradient should match"
    assert np.allclose(grads_nb[1].to_numpy(), grads_jax[1]), "Second gradient should match"
...
def test_nested_structure():
    ...
    assert set(grads_nb[0].keys()) == set(grads_jax[0].keys()), "Gradient dicts should have same keys"
    
    # Check values
    for key in grads_nb[0].keys():
        assert np.allclose(grads_nb[0][key].to_numpy(), grads_jax[0][key]), f"Gradient for {key} should match"

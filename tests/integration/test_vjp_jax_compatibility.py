...
def test_single_array_arg():
    ...
    if JAX_AVAILABLE:
        ...
        print(f"  Gradient length: {len(gradients_jax) if isinstance(gradients_jax, tuple) else 'N/A'}")
        
        # Compare values (both now return tuples)
        jax_grad_value = gradients_jax[0]
        nabla_grad_value = gradients_nabla[0]
        assert np.allclose(nabla_grad_value.to_numpy(), jax_grad_value), "Gradient values don't match!"
...
def test_multiple_array_args():
    ...
    if JAX_AVAILABLE:
        ...
        assert isinstance(gradients_nabla, tuple), "Nabla should return tuple for multiple args"
        assert isinstance(gradients_jax, tuple), "JAX should return tuple for multiple args"
        assert len(gradients_nabla) == len(gradients_jax), "Gradient tuple lengths should match"
        
        assert np.allclose(gradients_nabla[0].to_numpy(), gradients_jax[0]), "Gradient w.r.t. x doesn't match!"
        assert np.allclose(gradients_nabla[1].to_numpy(), gradients_jax[1]), "Gradient w.r.t. y doesn't match!"
...
def test_dict_input():
    ...
    print(f"  Output: {outputs_nabla}")
    print(f"  Gradient type: {type(gradients_nabla)}")
    print(f"  Gradient keys: {list(gradients_nabla.keys()) if isinstance(gradients_nabla, dict) else 'N/A'}")
    ...
    if JAX_AVAILABLE:
        ...
        assert isinstance(nabla_grad_dict, dict), "Nabla should return dict inside tuple"
        assert isinstance(jax_grad_dict, dict), "JAX should return dict inside tuple"
        assert set(nabla_grad_dict.keys()) == set(jax_grad_dict.keys()), "Gradient dict keys should match"
        
        assert np.allclose(nabla_grad_dict["x"].to_numpy(), jax_grad_dict["x"]), "Gradient w.r.t. x doesn't match!"
        assert np.allclose(nabla_grad_dict["y"].to_numpy(), jax_grad_dict["y"]), "Gradient w.r.t. y doesn't match!"
        print("  ✓ Gradient structures and values match perfectly!")
...
def test_nested_dict_input():
    ...
    if isinstance(gradients_nabla, tuple) and len(gradients_nabla) > 0:
        nabla_grad_dict = gradients_nabla[0]
        print(f"  Gradient top-level keys: {list(nabla_grad_dict.keys()) if isinstance(nabla_grad_dict, dict) else 'N/A'}")
    ...
    if JAX_AVAILABLE:
        ...
        assert isinstance(gradients_nabla, tuple), "Nabla should return tuple"
        assert isinstance(nabla_grad_dict, dict), "Nabla should preserve dict structure inside tuple"
        assert "layer1" in nabla_grad_dict, "Nabla should preserve nested structure"
        assert set(nabla_grad_dict["layer1"].keys()) == set(jax_grad_dict["layer1"].keys()), "Nested keys should match"
        
        assert np.allclose(nabla_grad_dict["layer1"]["weights"].to_numpy(), 
                          jax_grad_dict["layer1"]["weights"]), "Gradient w.r.t. weights doesn't match!"
        assert np.allclose(nabla_grad_dict["layer1"]["bias"].to_numpy(), 
                          jax_grad_dict["layer1"]["bias"]), "Gradient w.r.t. bias doesn't match!"
...
def test_list_input():
    ...
    if JAX_AVAILABLE:
        ...
        assert isinstance(gradients_nabla, tuple), "Nabla should return tuple"
        assert isinstance(nabla_grad_list, list), "Nabla should preserve list structure inside tuple"
        ...
        assert np.allclose(nabla_grad_list[0].to_numpy(), jax_grad_list[0]), "Gradient[0] doesn't match!"
        assert np.allclose(nabla_grad_list[1].to_numpy(), jax_grad_list[1]), "Gradient[1] doesn't match!"
...
def test_mixed_nested_structure():
    ...
    if isinstance(gradients_nabla, tuple) and len(gradients_nabla) > 0:
        nabla_grad_dict = gradients_nabla[0]
        print(f"  Gradient dict keys: {list(nabla_grad_dict.keys()) if isinstance(nabla_grad_dict, dict) else 'N/A'}")
    ...
    if JAX_AVAILABLE:
        ...
        assert isinstance(gradients_nabla, tuple), "Nabla should return tuple"
        assert isinstance(nabla_grad_dict, dict), "Nabla should preserve dict structure inside tuple"
        assert isinstance(nabla_grad_dict['y'], list), "Nabla should preserve nested list structure"
        assert len(nabla_grad_dict['y']) == len(jax_grad_dict['y']), "Nested list lengths should match"
        
        # Check values
        assert np.allclose(nabla_grad_dict["x"].to_numpy(), jax_grad_dict["x"]), "Gradient w.r.t. x doesn't match!"
        assert np.allclose(nabla_grad_dict["y"][0].to_numpy(), jax_grad_dict["y"][0]), "Gradient w.r.t. y[0] doesn't match!"
        assert np.allclose(nabla_grad_dict["y"][1].to_numpy(), jax_grad_dict["y"][1]), "Gradient w.r.t. y[1] doesn't match!"
...
def summarize_findings():
    ...
    print("   • Multiple args: nabla.vjp(f, x, y) returns (output, lambda: (grad_x, grad_y))")
    print("   • Pytree args: nabla.vjp(f, pytree) returns (output, lambda: (grad_pytree,))")

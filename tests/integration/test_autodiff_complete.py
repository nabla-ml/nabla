...
def test_jvp_of_vjp():
    ...
    assert abs(hessian_at_3 - 2.0) < 1e-6, f"Second derivative incorrect: {hessian_at_3}"
...
def test_vjp_of_jvp():
    ...
    assert abs(mixed_deriv_at_2 - 12.0) < 1e-6, f"Mixed derivative incorrect: {mixed_deriv_at_2}"
...
def test_double_nested_transformations():
    ...
    # At x=2: 24*2 = 48
    assert abs(result_value - 48.0) < 1e-6, f"VJP(JVP(f)) value incorrect: {result_value}"
    assert abs(result_tangent - 48.0) < 1e-6, f"JVP(VJP(JVP(f))) value incorrect: {result_tangent}"
...
def test_multivariable_nested():
    ...
    partial_x_values, mixed_partial = nb.jvp(partial_x_fn, [x, y], [tangent_x, tangent_y])
    ...
    assert abs(partial_x_at_2_3 - 12.0) < 1e-6, f"Partial derivative incorrect: {partial_x_at_2_3}"
    assert abs(mixed_partial_at_2_3 - 4.0) < 1e-6, f"Mixed partial incorrect: {mixed_partial_at_2_3}"

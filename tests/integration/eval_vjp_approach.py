...
def test_edge_cases():
    ...
    try:
        ...
        print(
            f"       Deep weights grad: {grad_kwargs['level1']['level2']['level3']['weights']}"
        )
        print(f"       Deep other grad: {grad_kwargs['level1']['level2']['other'][0]}")
        print(
            f"       Metadata preserved: {grad_kwargs['level1']['level2']['level3']['metadata']}"
        )
    except Exception as e:
    ...
    try:
        ...
        print("   ⚠️  Circular reference: Known limitation (recursion)")
    except Exception as e:
    ...
    try:
        ...
        print(
            f"       All gradients computed: {all(k in grad_kwargs for k in large_dict)}"
        )
    except Exception as e:

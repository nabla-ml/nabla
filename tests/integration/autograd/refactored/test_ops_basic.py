...
def test_unary_ops_vjp():
    ops = [
        ...
        ("sigmoid", nb.ops.sigmoid, pytest.importorskip("jax.nn").sigmoid if HAS_JAX else None),
...
def test_reduction_ops_vjp():
    ...
    cases = [
        ("sum_axis0", lambda x: nb.reduce_sum(x, axis=0), lambda x: jnp.sum(x, axis=0) if HAS_JAX else None),
        ("sum_axis1_keepdims", lambda x: nb.reduce_sum(x, axis=1, keepdims=True), lambda x: jnp.sum(x, axis=1, keepdims=True) if HAS_JAX else None),
        ("mean_axis0", lambda x: nb.mean(x, axis=0), lambda x: jnp.mean(x, axis=0) if HAS_JAX else None),
        ("max_axis0", lambda x: nb.reduce_max(x, axis=0), lambda x: jnp.max(x, axis=0) if HAS_JAX else None),
    ]
...
def test_matmul_vjp():
    ...
    check_vjp(
        "matmul", 
        nb.matmul, 
        jnp.matmul if HAS_JAX else None, 
        (x, y)
    )

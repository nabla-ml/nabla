...
def pipeline_step_fn(current_activations, w_stage):
    ...
    return lax.ppermute(y, axis_name='stage', perm=[(i, (i + 1) % STAGES) for i in range(STAGES)])

pipeline_step = shard_map(pipeline_step_fn, mesh=mesh, in_specs=(P('stage', None, None), P('stage', None, None)), out_specs=P('stage', None, None))
...
def pipeline_loss_fn(all_inputs, weights, targets):
    ...
    def scan_body(pipe_state, fresh_input):
        ...
        next_pipe = next_pipe.at[0].set(fresh_input) # Overwrite garbage with fresh data
...
if diff_w < 1e-3 and diff_x < 1e-3:
    ...
    print("SUCCESS: The sharded pipeline produces MATHEMATICALLY IDENTICAL gradients to the sequential model.")
else:

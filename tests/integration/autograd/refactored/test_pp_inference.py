...
def pipeline_inference_loop(padded_inputs, weight_stack, current_state, mask_0, step_fn, perm, total_steps):
    ...
    for t in range(total_steps):
        ...
        current_state, res = pipeline_step(current_state, fresh, weight_stack, mask_0, step_fn, perm)
        results.append(res)
...
def test_pp_inference_clean():
    ...
    step_fn = vmap(stage_compute, in_axes=(0, 0), out_axes=0, spmd_axis_name="stage", mesh=mesh)
    
    # Wrap loop for tracing
    def trace_wrapper(inputs, weights, state, mask):
        total_steps = MICRO_BATCHES + STAGES
        return pipeline_inference_loop(inputs, weights, state, mask, step_fn, perm, total_steps)

    # --- Run ---
    traced = nb.core.graph.tracing.trace(trace_wrapper, x_padded_nb, w_sharded, state_sharded, mask_0_sharded)
    ...
    for i in range(MICRO_BATCHES):
        act = x_np[i]
        for s in range(STAGES): act = np.maximum(act @ w_np[s], 0)
        outs.append(act)
    ref = np.stack(outs)

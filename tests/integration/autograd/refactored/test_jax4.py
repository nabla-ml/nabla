...
def step_core_fn(state, inputs, constants):
    ...
    return (grads, stash, w_idx, r_idx), \
           (fwd_out, fwd_tgt, fwd_msk, d_x, do_bwd)
...
def step_1f1b_adapter(state, inputs, constants):
    ...
    return new_state, (fwd_out_shifted, fwd_tgt_shifted, fwd_msk_shifted, bwd_out_shifted, bwd_msk_shifted)
...
x_pad = jnp.concatenate([x_data_global, jnp.zeros((DP, pad_len, MICRO_BATCH_SIZE, DIM))], axis=1)
y_pad = jnp.concatenate([y_data_global, jnp.zeros((DP, pad_len, MICRO_BATCH_SIZE, DIM))], axis=1)

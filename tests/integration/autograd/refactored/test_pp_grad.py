...
def pipeline_loop(padded_inputs, weight_stack, current_state, mask_0, step_fn, perm, total_steps):
...
def test_pp_grad_clean():
    ...
    def pipeline_loss(inputs, weights, state, mask, targets):
        ...
        stream_outputs, _ = pipeline_loop(inputs, weights, state, mask, step_fn, perm, total_steps)
    ...
    w_grad_sharded = grad_fn(x_padded_nb, w_sharded, state_sharded, mask_0_sharded, y_nb)
    ...
    def jax_ref(params, x, y):
        def apply(curr, w): return jax.nn.relu(curr @ w)
        preds = []
        for i in range(MICRO_BATCHES):
            a = x[i]
            for w in params: a = apply(a, w)
            preds.append(a)

...
def assert_grads_match(nabla_fn, jax_fn, inputs_np, mesh, mesh_dim_specs, name, vmapping=None):
    ...
    if vmapping:
        ...
        nabla_target = lambda *args: nb.ops.reduce_sum(nabla_fn(*args), axis=list(range(len(nabla_fn(*args).shape))))
    ...
    for i, (gnb, gjax) in enumerate(zip(grads_nb, grads_jax)):
        gnb_np = to_np(gnb)
        np.testing.assert_allclose(gnb_np, gjax, rtol=1e-5, atol=1e-6, 
...
def test_all_ops_sharded_vjp():
    ...
    for name, n_op, j_op in unary_ops:
        ...
        assert_grads_match(n_op, j_op, (x_np,), mesh, ([["tp"], []],), name, vmapping=(0, 0))
    ...
    for name, n_op, j_op in binary_ops:
        ...
        assert_grads_match(n_op, j_op, (x_np, y_np), mesh, ([["tp"], []], [["tp"], []]), name, vmapping=(0, 0))
    ...
    assert_grads_match(nb.ops.matmul, jnp.matmul, (x_np, y_np), mesh, 
                       ([["tp"], [], []], [["tp"], [], []]), "matmul", vmapping=(0, 0))

    # 4. Reduction (inside vmap)
    # vmap(reduce_sum)
    def n_sum(x): return nb.ops.reduce_sum(x, axis=0) # reduce over rows of (4, 4)
    def j_sum(x): return jnp.sum(x, axis=0)
    x_np = np.random.randn(8, 4, 4).astype(np.float32)
    assert_grads_match(n_sum, j_sum, (x_np,), mesh, ([["tp"], [], []],), "reduce_sum_vmap", vmapping=(0, 0))

    # 5. View Ops
    # unsqueeze
    def n_unsq(x): return nb.ops.unsqueeze(x, axis=1)
    def j_unsq(x): return jnp.expand_dims(x, axis=1)
    x_np = np.random.randn(10, 5).astype(np.float32) # vmap(10), inner(5)
    assert_grads_match(n_unsq, j_unsq, (x_np,), mesh, ([["tp"], []],), "unsqueeze_vmap", vmapping=(0, 0))

    # squeeze
    def n_sq(x): return nb.ops.squeeze(x, axis=0)
    def j_sq(x): return jnp.squeeze(x, axis=0)
    x_np = np.random.randn(8, 1, 4).astype(np.float32)
    assert_grads_match(n_sq, j_sq, (x_np,), mesh, ([["tp"], [], []],), "squeeze_vmap", vmapping=(0, 0))

    # swap_axes
    def n_swap(x): return nb.ops.swap_axes(x, 0, 1)
    def j_swap(x): return jnp.swapaxes(x, 0, 1)
    x_np = np.random.randn(8, 4, 5).astype(np.float32)
    assert_grads_match(n_swap, j_swap, (x_np,), mesh, ([["tp"], [], []],), "swap_axes_vmap", vmapping=(0, 0))

    # 6. Softmax (Composition)
    from nabla.ops.unary import softmax
    def j_softmax(x): return jax.nn.softmax(x, axis=-1)
    x_np = np.random.randn(8, 16).astype(np.float32)
    assert_grads_match(softmax, j_softmax, (x_np,), mesh, ([["tp"], []],), "softmax_vmap", vmapping=(0, 0))
    ...
    def j_reshard(x): return x # identity in JAX
    x_np = np.random.randn(8, 4).astype(np.float32)
    assert_grads_match(n_reshard, j_reshard, (x_np,), mesh, ([["tp"], []],), "reshard_vmap", vmapping=(0, 0))

...
def check_vjp(fn, jax_fn, *args):
    ...
    if isinstance(nabla_out, (list, tuple)):
        cotangent = [
            nb.Tensor.from_dlpack(np.ones(tuple(int(d) for d in t.shape), dtype=np.float32)) 
            for t in nabla_out
        ]
    else:
...
def test_vjp_view():
    ...
    check_vjp(lambda a: nb.unsqueeze(a, axis=0), lambda a: jnp.expand_dims(a, axis=0), x)

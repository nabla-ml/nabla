...
def test_dp_vs_mp_grads():
    ...
    X_dp = nb.ops.shard(nb.Tensor.from_dlpack(X_data), mesh, [DimSpec(["tp"]), DimSpec([])])
    ...
    X_mp = nb.ops.shard(nb.Tensor.from_dlpack(X_data), mesh, [DimSpec([]), DimSpec(["tp"])])
    W_mp = nb.ops.shard(nb.Tensor.from_dlpack(W_data), mesh, [DimSpec(["tp"]), DimSpec([])])
    ...
    np.testing.assert_allclose(grad_dp[W_dp].to_numpy(), grad_mp[W_mp].to_numpy(), rtol=1e-5)
    np.testing.assert_allclose(grad_dp[X_dp].to_numpy(), grad_mp[X_mp].to_numpy(), rtol=1e-5)

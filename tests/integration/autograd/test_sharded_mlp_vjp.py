...
def test_sharded_mlp_grads():
    ...
    W = nb.ops.shard(nb.Tensor.from_dlpack(W_data), mesh, [DimSpec([]), DimSpec(["tp"])])
    ...
    assert not needs_reshard(grad_W.sharding, W.sharding), f"W grad sharding mismatch: {grad_W.sharding} vs {W.sharding}"
    assert not needs_reshard(grad_X.sharding, X.sharding), f"X grad sharding mismatch: {grad_X.sharding} vs {X.sharding}"

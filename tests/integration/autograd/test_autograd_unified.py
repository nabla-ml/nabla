    ...
    def get_pos_unary_args():
        return (
            nb.Tensor.from_dlpack(
                np.abs(np.random.randn(8, 8).astype(np.float32)) + 0.1
            ),
        )
...
def test_reduction_ops(axis, keepdims):
    ...
    ops = [
        (
            "sum",
            lambda x: nb.reduce_sum(x, axis=axis, keepdims=keepdims),
            lambda x: jnp.sum(x, axis=axis, keepdims=keepdims),
        ),
        (
            "mean",
            lambda x: nb.mean(x, axis=axis, keepdims=keepdims),
            lambda x: jnp.mean(x, axis=axis, keepdims=keepdims),
        ),
        (
            "max",
            lambda x: nb.reduce_max(x, axis=axis, keepdims=keepdims),
...
def test_softmax(axis):
    ...
    case = AutogradTestCase(
        "softmax",
        lambda x: nb.ops.softmax(x, axis=axis),

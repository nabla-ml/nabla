...
def test_nested_vmap_batched_matmul():
    ...
    def simple_batched_matmul(batch_matrices: nb.Array, single_matrix: nb.Array) -> nb.Array:
    ...
    try:
        ...
        print(f"Simple matmul: {mat_a.shape} @ {mat_b.shape} -> {simple_result[0].shape}")
        ...
        batch_size = 2
        batch_a = nb.arange((batch_size, 2, 3), nb.DType.float32)  # Batch of 2x3 matrices
        ...
        print(f"✓ Successfully computed batched matmul: {batch_a.shape} @ {mat_b.shape} -> {result.shape}")
        ...
        def enhanced_batched_matmul(args: list[nb.Array]) -> list[nb.Array]:
            ...
            result = nb.vmap(matrix_multiply_single, in_axes=0, out_axes=0)(batch_matrices)
            return [result]
...
def test_advanced_nested_vmap_patterns():
    ...
    try:
        ...
        def compute_all_distances(a_batch, b_batch):
            ...
            distances = nb.pow(nb.sum(diff * diff, axes=[-1]), 0.5)  # Using pow(x, 0.5) for sqrt
            return distances
        ...
        def process_batch_element(x, weights):
            ...
            """
            print(f"Debug - process_batch_element: x.shape={x.shape}, weights.shape={weights.shape}")
            print(f"Debug - x.batch_dims={x.batch_dims}, weights.batch_dims={weights.batch_dims}")
            result = nb.sum(x * weights, axes=[1])  # Sum over feature dimension, keep batch dimension
        ...
        simple_result = nb.vmap(simple_test_func, in_axes=[0, None])([batch_data, shared_weights])
        ...
        # Verify with manual computation
        expected_results = np.sum(batch_data.to_numpy() * shared_weights.to_numpy(), axis=1)

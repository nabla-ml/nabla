...
def test_nested_dict_vjp():
    ...
    def simple_forward(inputs_dict):
        ...
        l1_out = add(mul(inputs_dict['layer1']['weights'], 2.0), inputs_dict['layer1']['bias'])
        l2_out = add(mul(inputs_dict['layer2']['weights'], 3.0), inputs_dict['layer2']['bias'])
    ...
    print(f"   Input structure preserved: {sorted(grad_dict.keys()) == sorted(inputs.keys())}")
    print(f"   Layer1 structure preserved: {sorted(grad_dict['layer1'].keys()) == sorted(inputs['layer1'].keys())}")
    print(f"   Layer2 structure preserved: {sorted(grad_dict['layer2'].keys()) == sorted(inputs['layer2'].keys())}")
    print(f"   Non-array value preserved: {grad_dict['learning_rate'] == inputs['learning_rate']}")
    print(f"   Gradients computed: weights=[2,2], bias=[2] (as expected)")
...
def test_mixed_types_vjp():
    ...
    def mixed_function(scale_int, scale_float, data_list, config_dict, x_array, y_array):
    ...
    # Compute VJP
    output, vjp_fn = vjp(mixed_function, scale_int, scale_float, data_list, config_dict, x_array, y_array)
    ...
    grad_scale_int, grad_scale_float, grad_data_list, grad_config_dict, grad_x_array, grad_y_array = gradients
    ...
    assert sorted(grad_config_dict['model'].keys()) == sorted(config_dict['model'].keys())

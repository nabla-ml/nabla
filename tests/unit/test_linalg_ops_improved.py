...
from tests.unit.test_utils import JAX_AVAILABLE, generate_test_data, allclose_recursive
...
class TestMatmulOperation:
    ...
    def test_matmul_forward_pass(self, dtype, shape_a, shape_b):
        ...
        # Execute
        result_nb = nb.matmul(
            nb.Array.from_numpy(a_np), 
            nb.Array.from_numpy(b_np)
        )
    ...
    def test_matmul_vjp_correctness(self, dtype):
        ...
        for i, (grad_nb, grad_jax) in enumerate(zip(grads_nb, grads_jax)):
    ...
    def test_matmul_jvp_correctness(self, dtype):
        ...
        # Setup
        primals_np = [generate_test_data(shape_a, dtype), generate_test_data(shape_b, dtype)]
        tangents_np = [generate_test_data(shape_a, dtype), generate_test_data(shape_b, dtype)]
        ...
        jax_op = lambda x, y: jnp.matmul(x, y)
        primal_out_jax, tangent_out_jax = jax.jvp(jax_op, tuple(primals_jax), tuple(tangents_jax))
        ...
        assert allclose_recursive(tangent_out_nb[0], tangent_out_jax, rtol_tan, atol_tan)

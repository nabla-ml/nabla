...
# =============================================================================
# Basic Sharding Inside vmap - Unary Operations
# =============================================================================
...
class TestVmapWithShardingUnaryOps:
    ...
    def test_vmap_relu_with_sharding_inside(self, mesh_shape, mesh_axes):
        ...
        def f(x):  # x has LOGICAL shape (8, 16)
            # User applies sharding thinking about logical dims
            x_sharded = x.shard(mesh, P(None, mesh_axes[-1]))  # shard last dim on last mesh axis
            return relu(x_sharded)
        
        # Input: (batch=4, 8, 16) - UNSHARDED
    ...
    def test_vmap_sigmoid_with_feature_sharding(self, mesh_shape, mesh_axes):
        ...
        def f(x):  # x has LOGICAL shape (16,)
            x_sharded = x.shard(mesh, P(mesh_axes[-1]))  # shard feature dim
            return sigmoid(x_sharded)
        
        np_x = make_array(batch, 16, seed=42) * 0.1  # Small values for sigmoid
        x = tensor_from_numpy(np_x)
...
# =============================================================================
# Binary Operations with Broadcasting - Realistic Patterns
# =============================================================================
...
class TestVmapBinaryOpsWithSharding:
    ...
    def test_vmap_add_batched_data_sharded_bias(self, mesh_shape, mesh_axes):
        ...
        def f(x, bias):  # x: (hidden,), bias: (hidden,) - both LOGICAL shapes
            # Shard bias on tensor parallel axis
            bias_sharded = bias.shard(mesh, P(mesh_axes[-1]))
            # Add will broadcast bias across batch automatically
        ...
        # vmap over x (batched), broadcast bias
    ...
    def test_vmap_mul_elementwise_with_sharding(self, mesh_shape, mesh_axes):
        ...
        def f(x, y):  # Both have LOGICAL shape (8, 16)
...
# =============================================================================
# Matmul - Megatron-Style Column and Row Parallel
# =============================================================================
...
class TestVmapMatmulSharding:
    ...
    def test_vmap_matmul_column_parallel(self):
        ...
        def f(x, w):  # x: (in_features,), w: (in_features, out_features)
            # Shard weights on output dimension (column parallel)
            w_sharded = w.shard(mesh, P(None, "tp"))
            result = matmul(x, w_sharded)
            # Result inherits sharding on last dim
            return result
    ...
    def test_vmap_matmul_row_parallel(self):
        ...
        def f(x, w):  # x: (in_features,), w: (in_features, out_features)
            # Input x should be sharded on last dim (from previous layer)
            x_sharded = x.shard(mesh, P("tp"))
            # Shard weights on input dimension (row parallel)
            w_sharded = w.shard(mesh, P("tp", None))
            result = matmul(x_sharded, w_sharded)
            # Result will need AllReduce since contracting dim is sharded
            return result
    ...
    def test_vmap_matmul_2d_mesh_hybrid_parallel(self):
        ...
        def f(x, w):  # x: (in_features,), w: (in_features, out_features)
            # Shard weights on tensor parallel axis (column parallel)
            w_sharded = w.shard(mesh, P(None, "tp"))
            result = matmul(x, w_sharded)
            return result
...
# =============================================================================
# Reduction Operations with Sharding
# =============================================================================
...
# =============================================================================
# Composite Functions - Realistic MLP Scenarios
# =============================================================================
...
class TestVmapCompositeWithSharding:
    ...
    def test_mlp_layer_megatron_style(self):
        ...
        def mlp_layer(x, w1, w2, b1):
            # x: (hidden,), w1: (hidden, ffn), b1: (ffn,), w2: (ffn, hidden)
            
            # Layer 1: Column parallel
            w1_sharded = w1.shard(mesh, P(None, "tp"))
            h = matmul(x, w1_sharded)
            
            # Bias add (bias is also sharded to match)
            ...
            # Layer 2: Row parallel (contracting dim sharded -> AllReduce)
    ...
    def test_attention_qk_sharding(self):
        ...
        def compute_attention_scores(q, k):
            # q, k: (seq_len, hidden)
            # Shard on hidden dimension (tensor parallel)
            q_sharded = q.shard(mesh, P(None, "tp"))
            k_sharded = k.shard(mesh, P(None, "tp"))
            
            # Transpose k: (hidden, seq_len)
            k_t = swap_axes(k_sharded, 0, 1)
            
            # QK^T: (seq_len, hidden) @ (hidden, seq_len) -> (seq_len, seq_len)
            # Contracting dim (hidden) is sharded -> AllReduce
...
class TestNestedVmapWithSharding:
    ...
    def test_nested_vmap_with_sharding(self):
        ...
        def inner_fn(x):  # x: (features,)
            x_sharded = x.shard(mesh, P("tp"))
            return relu(x_sharded)
        
        def outer_fn(x_batch):  # x_batch: (inner_batch, features)
...
# =============================================================================
# COMPREHENSIVE: All Unary Operations
# =============================================================================
...
class TestVmapShardingAllUnaryOps:
    ...
    def test_unary_op_with_sharding(self, op_name, nabla_op, numpy_op, mesh_shape, mesh_axes):
...
# =============================================================================
# COMPREHENSIVE: All Binary Operations
# =============================================================================
...
class TestVmapShardingAllBinaryOps:
    ...
    def test_binary_op_both_batched_sharded(self, op_name, nabla_op, numpy_op, mesh_shape, mesh_axes):
    ...
    def test_binary_op_batched_broadcast(self, op_name, nabla_op, numpy_op, mesh_shape, mesh_axes):
        """Test binary op with batched input + broadcast weight (realistic pattern)."""
        batch = 4
        features = 16
        mesh = DeviceMesh("mesh", mesh_shape, mesh_axes)
        
        def f(x, w):  # x: (features,), w: (features,)
...
# =============================================================================
# COMPREHENSIVE: All View Operations
# =============================================================================
...
class TestVmapShardingAllViewOps:
    ...
    def test_squeeze_with_sharding(self, mesh_shape, mesh_axes):
        ...
        def f(x):  # x has LOGICAL shape (1, 16)
            x_sharded = x.shard(mesh, P(None, mesh_axes[-1]))
...
# =============================================================================
# COMPREHENSIVE: All Reduction Operations
# =============================================================================
...
class TestVmapShardingAllReductionOps:
    ...
    def test_reduce_sum_axis1(self, mesh_shape, mesh_axes):
        ...
        def f(x):  # x has LOGICAL shape (8, 16)
            x_sharded = x.shard(mesh, P(None, mesh_axes[-1]))
            return reduce_sum(x_sharded, axis=1)  # Reducing sharded dim
...
# =============================================================================
# COMPREHENSIVE: All Matmul Patterns
# =============================================================================
...
class TestVmapShardingAllMatmulPatterns:
    ...
    def test_matmul_both_weights_sharded(self, mesh_shape, mesh_axes):
        ...
        def f(x, w1, w2):  # x: (H,), w1: (H, FFN), w2: (FFN, H)
            w1_sharded = w1.shard(mesh, P(None, mesh_axes[-1]))  # Column parallel
            h = matmul(x, w1_sharded)  # h: (FFN,) sharded on tp
            
            w2_sharded = w2.shard(mesh, P(mesh_axes[-1], None))  # Row parallel
            out = matmul(h, w2_sharded)  # AllReduce needed
            return out
...
# =============================================================================
# COMPREHENSIVE: Multi-output Operations
# =============================================================================
...
class TestVmapShardingMultiOutputOps:
    ...
    def test_split_with_sharding(self, mesh_shape, mesh_axes):
        ...
        def f(x):  # x has LOGICAL shape (16,)
            x_sharded = x.shard(mesh, P(mesh_axes[-1]))
            return split(x_sharded, num_splits=2, axis=0)  # Split into 2 parts
        ...
        assert len(result) == 2
        for i, (r, e) in enumerate(zip(result, expected_parts)):

...
# 1. Enable Debug Printing BEFORE importing nabla
...
def compute_logic(x):
    # y is an intermediate. 
    # In function scope, it exists.
    # Once returned, if not returned, it dies.
    y = x * x
    z = x + x
    return z
...
def test_traced():
    ...
    print(">> Calling z.realize()...")
    print("EXPECTATION: Only 'z' should be evaluated. 'y' is alive via refs but should be skipped by current fix.")
    z.realize()
...
def test_long_chain_trace():
    ...
    # Expect: 3 constants (2.0, 10.0, 2.0) + 5 ops (mul, add, div, sub, mul) = 8
    assert len(t.nodes) == 8, f"Expected 8 ops, got {len(t.nodes)}"
    print("✅ Logic Verified")
    
    # NEW: Verify MAX Graph execution
    print("\n>> Realizing output to check MAX Graph...")
    print("EXPECTATION: Graph should contain the full chain but ONLY 'v5' (and maybe 'x') as outputs.")
    print("             Intermediates (v1..v4) should NOT be outputs.")
    
    # trace() returns a Trace object. t.outputs is the result of the function.
    res = t.outputs
...
def test_alive_intermediate():
    ...
    # v1 is intermediate. We KEEP a reference to it.
    v1 = x * 2.0
    v2 = v1 + 5.0
    
    print(">> Calling v2.realize()...")
    print("EXPECTATION: Both 'v2' AND 'v1' should be evaluated because 'v1' is alive in Python scope.")
...
def test_multi_return():
    ...
    print("EXPECTATION: 'res_b' should ALSO be evaluated because it is alive/unrealized in this scope.")

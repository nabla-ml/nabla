...
class TestControlFlow(unittest.TestCase):
    def setUp(self):
        # Ensure graph context if needed? Nabla operations usually run in global graph.
        pass
    ...
    def test_scan_cumsum(self):
        # scan over [1, 2, 3, 4] -> cumsum [1, 3, 6, 10]
        def f(carry, x):
            # x is a single element tensor here
            new_carry = carry + x
        ...
        np.testing.assert_array_equal(stacked_ys.to_numpy(), np.array([1, 3, 6, 10], dtype=np.float32))

    def test_scan_nested(self):
        # Test scan with nested input structure
        # xs = ( [1,2], [10, 20] )
        # f takes (x_a, x_b) adds them
        pass

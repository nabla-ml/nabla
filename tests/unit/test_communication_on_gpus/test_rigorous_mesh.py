...
@pytest.mark.parametrize(
    "mesh_shape, mesh_axes, scenario_name",
    [
        ((2,), ("x",), "2_Linear"),
        ((4,), ("x",), "4_Linear"),
        ((8,), ("x",), "8_Linear"),
...
def test_scenario(mesh_shape, mesh_axes, scenario_name):
    ...
    try:
        mesh = DeviceMesh(
            f"mesh_{scenario_name}", shape=mesh_shape, axis_names=mesh_axes
        )
    except Exception as e:
        pytest.skip(
            f"Skipping scenario, failed to create mesh (maybe > available devices?): {e}"
        )

...
from nabla import (
    ...
    reduce_sum, mean,
    reshape, squeeze, unsqueeze, swap_axes, broadcast_to, moveaxis,
)
...
class TestBinaryOpsBasic:
    ...
    def test_div(self, shape):
        ...
        np_a = make_array(*shape, seed=42)
        np_b = make_positive_array(*shape, seed=43)  # Avoid division by zero
...
class TestBinaryOpsBroadcasting:
    ...
    @pytest.mark.parametrize("shape_a,shape_b,expected_shape", [
        ((4, 8), (8,), (4, 8)),            # (4, 8) + (8,) → (4, 8)
        ((8,), (4, 8), (4, 8)),            # (8,) + (4, 8) → (4, 8)
        ((4, 1), (1, 8), (4, 8)),          # (4, 1) + (1, 8) → (4, 8)
        ((2, 4, 8), (8,), (2, 4, 8)),      # (2, 4, 8) + (8,) → (2, 4, 8)
        ((2, 4, 8), (4, 8), (2, 4, 8)),    # (2, 4, 8) + (4, 8) → (2, 4, 8)
        ((2, 1, 8), (4, 1), (2, 4, 8)),    # (2, 1, 8) + (4, 1) → (2, 4, 8)
    ])
...
class TestReductionOpsSharding:
    ...
    def test_reduce_sum_non_sharded_axis(self, mesh_1d):
        ...
        result = reduce_sum(x, axis=1)  # Reduce non-sharded axis
        expected = np.sum(np_x, axis=1)
        
        assert_shape(result, expected.shape)
        assert_is_sharded(result, True)  # Still sharded on axis 0
        assert_allclose(result, expected)
    ...
    def test_reduce_sum_sharded_axis(self, mesh_1d):
        ...
        result = reduce_sum(x, axis=0)  # Reduce sharded axis
...
class TestReshape:
    ...
    @pytest.mark.parametrize("shape,new_shape", [
        ...
        ((2, 4, 8), (8, 8)),      # 3D to 2D
        ((8, 8), (2, 4, 8)),      # 2D to 3D
        ((4, 8), (8, 4)),         # Same dims, different order
    ])
...
class TestSwapAxes:
    ...
    @pytest.mark.parametrize("shape,axis1,axis2,expected_shape", [
        ((4, 8), 0, 1, (8, 4)),
        ((2, 4, 8), 0, 1, (4, 2, 8)),
        ((2, 4, 8), 0, 2, (8, 4, 2)),
        ((2, 4, 8), 1, 2, (2, 8, 4)),
...
class TestMoveaxis:
    ...
    @pytest.mark.parametrize("shape,source,destination,expected_shape", [
        ((4, 8), 0, 1, (8, 4)),
        ((4, 8), 1, 0, (8, 4)),
        ((2, 4, 8), 0, 2, (4, 8, 2)),
        ((2, 4, 8), 2, 0, (8, 2, 4)),
...
class TestViewOpsSharding:
    ...
    def test_swap_axes_sharded(self, mesh_1d):
        ...
        assert_shape(result, (4, 8))
        assert_is_sharded(result, True)  # Sharding should swap too
        assert_allclose(result, expected)

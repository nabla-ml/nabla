...
def test_argmax():
    ...
    grad_jax_vjp = vjp_fn_jax(jnp.ones_like(value_jax_vjp))
    print(
        "JAX ArgMax VJP Gradient:", grad_jax_vjp[0]
    )  # Note: JAX returns tuple for gradients
    ...
    print("VJP Gradient shapes - Nabla:", grad_vjp.shape, "JAX:", grad_jax_vjp[0].shape)
    print(
        "VJP gradients match (both should be zeros):",
        np.allclose(grad_vjp.to_numpy(), grad_jax_vjp[0]),
    )
    ...
    print(
        "JVP gradients are zeros in Nabla (correct for argmax):",
        np.allclose(grad_jvp.to_numpy(), 0),
    )
...
def test_min_binary():
    ...
    # JAX JVP
    value_jax_jvp, grad_jax_jvp = jax.jvp(
        foo_jax, (a_jax, b_jax), (jnp.ones_like(a_jax), jnp.ones_like(b_jax))
    )
    ...
    print(
        "VJP gradients match:",
        np.allclose(grad[0].to_numpy(), grad_jax[0])
        and np.allclose(grad[1].to_numpy(), grad_jax[1]),
    )

...
def jax_transpose_wrapper(x, axis_1, axis_2):
    ...
    axis_1_pos, axis_2_pos = (axis_1 if axis_1 >= 0 else rank + axis_1), (axis_2 if axis_2 >= 0 else rank + axis_2)
...
def jax_unsqueeze_wrapper(x, axes):
    res = x
    for axis in sorted(axes): res = jnp.expand_dims(res, axis=axis)
    return res

def jax_squeeze_wrapper(x, axes): return jnp.squeeze(x, axis=tuple(axes))
def jax_slice_wrapper(x, slices): return x[tuple(slices)]
def jax_move_axis_wrapper(x, source, destination): return jnp.moveaxis(x, source, destination)
...
def jax_matmul_wrapper(x, y):
    ...
    squeeze_axes = []
    if x_rank == 1:
        x_jax = jnp.expand_dims(x, 0); squeeze_axes.append(-2)
    if y_rank == 1:
        y_jax = jnp.expand_dims(y, 1); squeeze_axes.append(-1)
...
def get_test_data_for_shapes(
    ...
    for i, shape in enumerate(shapes):
        ...
        if config.input_dtype == "bool":
            ...
            nb_val, jax_val = (nb.equal(nb.arange(num_elements).reshape(shape) % 2, 0), (jax_base.reshape(shape) % 2 == 0)) if shape else (nb.array(True), jnp.array(True))
        else: # Numeric
            if not shape:
                base_val = 2.5 if config.domain_positive else 1.5
                nb_val, jax_val = nb.array(base_val, dtype=nb.DType.float32), jnp.array(base_val, dtype="float32")
            else:
...
def get_tangents_for_jvp(primals_nb, primals_jax, config: OpConfig):
    if config.input_dtype == "bool":
        ...
        tangents_nb = tuple(nb.zeros_like(p).astype(nb.DType.float32) for p in primals_nb)
...
# --- UNARY OPERATIONS ---
unary_ops_data = [
    ("sin", nb.sin, jnp.sin, False), ("cos", nb.cos, jnp.cos, False), ("tanh", nb.tanh, jnp.tanh, False),
    ("log", nb.log, jnp.log, True), ("exp", nb.exp, jnp.exp, False), ("sqrt", nb.sqrt, jnp.sqrt, True),
    ("abs", nb.abs, jnp.abs, False), ("floor", nb.floor, jnp.floor, False), ("negate", nb.negate, jnp.negative, False),
    ("relu", nb.relu, jax.nn.relu, False), ("sigmoid", nb.sigmoid, jax.nn.sigmoid, False)
]
for name, nb_fn, jax_fn, domain_pos in unary_ops_data:
    configs = [OpConfig(f"Rank{r}", ranks=(r,), domain_positive=domain_pos) for r in [0, 1, 2, 3]]
    ALL_OPERATIONS[name] = Operation(name, "UNARY", nb_fn, jax_fn, configs, standard_get_args)

ALL_OPERATIONS["logical_not"] = Operation("logical_not", "UNARY", nb.logical_not, jnp.logical_not,
    [OpConfig(f"Rank{r}", ranks=(r,), input_dtype="bool") for r in [0, 1, 2, 3]], standard_get_args)

# --- BINARY OPERATIONS ---
binary_ops_data = [
    ("add", nb.add, jnp.add), ("mul", nb.mul, jnp.multiply), ("sub", nb.sub, jnp.subtract), ("div", nb.div, jnp.divide),
    ("floordiv", nb.floordiv, jnp.floor_divide), ("mod", nb.mod, jnp.mod), ("pow", nb.pow, jnp.power),
    ("greater_equal", nb.greater_equal, jnp.greater_equal), ("equal", nb.equal, jnp.equal), ("not_equal", nb.not_equal, jnp.not_equal),
    ("maximum", nb.maximum, jnp.maximum), ("minimum", nb.minimum, jnp.minimum)
]
binary_rank_combos = [(0,0), (1,1), (2,2), (3,3), (0,1), (0,2), (0,3), (1,2), (1,3), (2,3)]
for name, nb_fn, jax_fn in binary_ops_data:
    use_stable = name in ["div", "floordiv", "mod"]
    configs = [OpConfig(f"Ranks({r1},{r2})", ranks=(r1, r2), use_stable_floats=use_stable) for r1, r2 in binary_rank_combos]
    ALL_OPERATIONS[name] = Operation(name, "BINARY", nb_fn, jax_fn, configs, standard_get_args)

# --- REDUCTION OPERATIONS ---
reduction_params = [
    {"desc": "all-keep", "axes": None, "keep_dims": True, "min_rank": 1}, {"desc": "all-nokeep", "axes": None, "keep_dims": False, "min_rank": 1},
    {"desc": "axis0-keep", "axes": 0, "keep_dims": True, "min_rank": 1}, {"desc": "axis0-nokeep", "axes": 0, "keep_dims": False, "min_rank": 1},
    {"desc": "axis1-keep", "axes": 1, "keep_dims": True, "min_rank": 2}, {"desc": "axis1-nokeep", "axes": 1, "keep_dims": False, "min_rank": 2},
    {"desc": "axes_0,1-keep", "axes": (0, 1), "keep_dims": True, "min_rank": 2}, {"desc": "axis2-keep", "axes": 2, "keep_dims": True, "min_rank": 3},
    {"desc": "axes_0,2-nokeep", "axes": (0, 2), "keep_dims": False, "min_rank": 3},
]
for name, nb_fn, jax_fn in [("sum", nb.sum, jnp.sum), ("mean", nb.mean, jnp.mean), ("max", nb.max, jnp.max), ("argmax", nb.argmax, jnp.argmax)]:
    configs = []
    for rank in [1, 2, 3]:
        for p in reduction_params:
            if rank >= p["min_rank"]:
                if name == "argmax" and isinstance(p["axes"], tuple): continue
                configs.append(OpConfig(f"Rank{rank}-{p['desc']}", ranks=(rank,), params={"axes": p["axes"], "keep_dims": p["keep_dims"]}))
    ALL_OPERATIONS[name] = Operation(name, "REDUCTION", nb_fn, jax_fn, configs, standard_get_args)

# --- VIEW OPERATIONS ---
view_ops_data = [
    Operation("transpose", "VIEW", nb.transpose, jax_transpose_wrapper, [
        OpConfig("Rank2-Swap_last_two", ranks=(2,), params={"axis_1": -2, "axis_2": -1}),
        OpConfig("Rank3-Swap_first_last", ranks=(3,), params={"axis_1": 0, "axis_2": -1}),
    ], standard_get_args),
    Operation("permute", "VIEW", nb.permute, jnp.transpose, [
        OpConfig("Rank2-Reverse", ranks=(2,), params={"axes": (1, 0)}), OpConfig("Rank3-Reverse", ranks=(3,), params={"axes": (2, 1, 0)}),
    ], standard_get_args),
    Operation("move_axis_to_front", "VIEW", nb.move_axis_to_front, lambda x, axis: jax_move_axis_wrapper(x, axis, 0), [
        OpConfig("Rank2-Move_axis_1", ranks=(2,), params={"axis": 1}),
    ], standard_get_args),
    Operation("reshape", "VIEW", nb.reshape, jnp.reshape, [
        OpConfig("Flatten", ranks=(3,)), OpConfig("Unflatten", primal_shapes=((2*4*8,),)),
    ], get_reshape_args),
    Operation("broadcast_to", "VIEW", nb.broadcast_to, jnp.broadcast_to, [
        OpConfig("Vec_to_Mat", primal_shapes=((3,),)), OpConfig("Mat_to_Ten3D", primal_shapes=((2,3),)),
    ], get_broadcast_to_args),
    Operation("squeeze", "VIEW", nb.squeeze, jax_squeeze_wrapper, [
        OpConfig("Squeeze_middle", primal_shapes=((2, 1, 3),), params={"axes": [1]}),
    ], standard_get_args),
    Operation("unsqueeze", "VIEW", nb.unsqueeze, jax_unsqueeze_wrapper, [
        OpConfig("Unsqueeze_middle", ranks=(2,), params={"axes": [1]}),
    ], standard_get_args),
    Operation("array_slice", "VIEW", nb.array_slice, jax_slice_wrapper, [
        OpConfig("Slice_with_step", primal_shapes=((5, 6, 7),), params={"slices": [slice(None), slice(0, None, 2)]}),
    ], standard_get_args),
    Operation("pad", "VIEW", nb.pad, jax_pad_inverse_slice, [
        OpConfig("Pad_matrix", primal_shapes=((2, 2),), params={"slices": [slice(1, 3), slice(2, 4)], "target_shape": (4, 5)}),
    ], standard_get_args),
    Operation("concatenate", "VIEW", nb.concatenate, jnp.concatenate, [
        OpConfig("Concat_axis1", ranks=(2, 2), params={"axis": 1}, is_list_input=True),
    ], standard_get_args),
    Operation("stack", "VIEW", nb.stack, jnp.stack, [
        OpConfig("Stack_axis1", ranks=(2, 2), params={"axis": 1}, is_list_input=True),
    ], standard_get_args),
]
for op in view_ops_data: ALL_OPERATIONS[op.name] = op

# --- LINALG OPERATIONS ---
ALL_OPERATIONS["matmul"] = Operation("matmul", "LINALG", nb.matmul, jax_matmul_wrapper, [
    OpConfig("Vector_@_Vector", primal_shapes=((4,), (4,))), OpConfig("Matrix_@_Vector", primal_shapes=((3, 4), (4,))),
    OpConfig("Vector_@_Matrix", primal_shapes=((4,), (4, 5))), OpConfig("Matrix_@_Matrix", primal_shapes=((3, 4), (4, 5))),
...
def create_op_tests(operation: Operation):
    ...
    def get_configured_functions():
        (primals_nb_orig, params_nb), (primals_jax_orig, params_jax_raw) = operation.get_args(current_config)
        ...
        if operation.name == "permute":
            ...
            params_jax = {k.replace('axes', 'axis').replace('keep_dims', 'keepdims'): v for k, v in params_jax_raw.items()}
    ...
    def test_1_baseline():
        ...
        return run_test_with_consistency_check("Baseline", lambda: f_nb(*primals_nb), lambda: f_jax(*primals_jax))
    ...
    def test_3_jvp():
        """Test: jvp(f)"""
        f_nb, primals_nb, f_jax, primals_jax = get_configured_functions()
        tangents_nb, tangents_jax = get_tangents_for_jvp(primals_nb, primals_jax, current_config)
        return run_test_with_consistency_check("JVP", lambda: nb.jvp(f_nb, primals_nb, tangents_nb), lambda: jax.jvp(f_jax, primals_jax, tangents_jax))

    def test_4_vmap():
        """Test: vmap(f)"""
        if operation.name == 'pad': pytest.skip("vmap for pad is non-trivial")
        f_nb, primals_nb, f_jax, primals_jax = get_configured_functions()
        return run_test_with_consistency_check("VMAP", lambda: nb.vmap(f_nb)(*primals_nb), lambda: jax.vmap(f_jax)(*primals_jax))

    def test_5_jit():
        """Test: jit(f)"""
        f_nb, primals_nb, f_jax, primals_jax = get_configured_functions()
        return run_test_with_consistency_check("JIT", lambda: nb.djit(f_nb)(*primals_nb), lambda: jax.jit(f_jax)(*primals_jax))
    ...
    def test_6_jit_vjp():
        ...
        def vjp_wrapper_jax(*args):
            ...
            return val, vjp_fn(jnp.ones_like(val))
        return run_test_with_consistency_check("JIT(VJP)", lambda: nb.djit(vjp_wrapper_nb)(*primals_nb), lambda: jax.jit(vjp_wrapper_jax)(*primals_jax))
    ...
    def test_7_jit_jvp():
        ...
        return run_test_with_consistency_check("JIT(JVP)", lambda: jitted_jvp_nb(primals_nb, tangents_nb), lambda: jitted_jvp_jax(primals_jax, tangents_jax))

    def test_8_jit_vmap():
        """Test: jit(vmap(f))"""
        if operation.name == 'pad': pytest.skip("vmap for pad is non-trivial")
        f_nb, primals_nb, f_jax, primals_jax = get_configured_functions()
        return run_test_with_consistency_check("JIT(VMAP)", lambda: nb.djit(nb.vmap(f_nb))(*primals_nb), lambda: jax.jit(jax.vmap(f_jax))(*primals_jax))
    ...
    def test_9_vmap_vjp():
        ...
        def vjp_wrapper_jax(*args):
            val, vjp_fn = jax.vjp(f_jax, *args)
            return val, vjp_fn(jnp.ones_like(val))
        return run_test_with_consistency_check("VMAP(VJP)", lambda: nb.vmap(vjp_wrapper_nb)(*primals_nb), lambda: jax.vmap(vjp_wrapper_jax)(*primals_jax))
    ...
    def test_10_vmap_jvp():
        ...
        return run_test_with_consistency_check("VMAP(JVP)", lambda: nb.vmap(jvp_wrapper_nb)(*primals_nb), lambda: jax.vmap(jvp_wrapper_jax)(*primals_jax))
    ...
    def test_11_jit_vmap_vjp():
        ...
        def vjp_wrapper_jax(*args):
            val, vjp_fn = jax.vjp(f_jax, *args)
            return val, vjp_fn(jnp.ones_like(val))
        return run_test_with_consistency_check("JIT(VMAP(VJP))", lambda: nb.djit(nb.vmap(vjp_wrapper_nb))(*primals_nb), lambda: jax.jit(jax.vmap(vjp_wrapper_jax))(*primals_jax))
    ...
    def test_12_jit_vmap_jvp():
        ...
        def jvp_wrapper_jax(*args):
            _, tangents = get_tangents_for_jvp(args, args, current_config)
            return jax.jvp(f_jax, args, tangents)
        return run_test_with_consistency_check("JIT(VMAP(JVP))", lambda: nb.djit(nb.vmap(jvp_wrapper_nb))(*primals_nb), lambda: jax.jit(jax.vmap(jvp_wrapper_jax))(*primals_jax))
    ...
    def test_14_jvp_vjp():
        ...
        def first_grad_jax(*args):
            _val, vjp_fn = jax.vjp(f_jax, *args)
            return vjp_fn(jnp.ones_like(f_jax(*args)))
        return run_test_with_consistency_check("JVP(VJP)", lambda: nb.jvp(first_grad_nb, primals_nb, tangents_nb), lambda: jax.jvp(first_grad_jax, primals_jax, tangents_jax))

    def test_15_vjp_jvp():
        """Test: vjp(jvp(f))"""
        f_nb, primals_nb, f_jax, primals_jax = get_configured_functions()
        tangents_nb, tangents_jax = get_tangents_for_jvp(primals_nb, primals_jax, current_config)
        def jvp_wrapper_nb(*args): return nb.jvp(f_nb, args, tangents_nb)
        def jvp_wrapper_jax(*args): return jax.jvp(f_jax, args, tangents_jax)
        def nabla_fn():
    ...
    def test_16_jvp_jvp():
        ...
        def jvp_wrapper_nb(*args): return nb.jvp(f_nb, args, tangents_nb)
        def jvp_wrapper_jax(*args): return jax.jvp(f_jax, args, tangents_jax)
        return run_test_with_consistency_check("JVP(JVP)", lambda: nb.jvp(jvp_wrapper_nb, primals_nb, tangents_nb), lambda: jax.jvp(jvp_wrapper_jax, primals_jax, tangents_jax))
    ...
    def test_18_jvp_vmap():
        ...
        tangents_nb, tangents_jax = get_tangents_for_jvp(primals_nb, primals_jax, current_config)
        vmapped_f_nb, vmapped_f_jax = nb.vmap(f_nb), jax.vmap(f_jax)
        return run_test_with_consistency_check("JVP(VMAP)", lambda: nb.jvp(vmapped_f_nb, primals_nb, tangents_nb), lambda: jax.jvp(vmapped_f_jax, primals_jax, tangents_jax))
    ...
    def test_19_vmap_vmap():
        ...
        return run_test_with_consistency_check("VMAP(VMAP)", lambda: nb.vmap(nb.vmap(f_nb))(*primals_nb), lambda: jax.vmap(jax.vmap(f_jax))(*primals_jax))

    test_functions = [
        test_1_baseline, test_2_vjp, test_3_jvp, test_4_vmap, test_5_jit, test_6_jit_vjp, test_7_jit_jvp,
        test_8_jit_vmap, test_9_vmap_vjp, test_10_vmap_jvp, test_11_jit_vmap_vjp, test_12_jit_vmap_jvp,
        test_13_vjp_vjp, test_14_jvp_vjp, test_15_vjp_jvp, test_16_jvp_jvp, test_17_vjp_vmap, test_18_jvp_vmap, test_19_vmap_vmap,
    ]

    def set_config(config): nonlocal current_config; current_config = config
    ...
    for config in op_def.configs:
        for trans_idx in range(19):
            ...
            ALL_TEST_CASES.append(
                pytest.param(op_name, config, trans_idx, id=test_id)
            )
...
def test_unified_operation_transformation(operation_name, config, transformation_index):
    ...
    test_desc = test_func.__doc__.split(":")[1].strip() if test_func.__doc__ else f"transform_{transformation_index}"
    ...
    for config in operation.configs:
        ...
        for i, test_func in enumerate(test_functions):
            ...
            try:
                ...
                traceback.print_exc()
                total_run += 1
                failed_tests.append(f"Config '{config.description}', Test '{desc}' (ERROR: {e})")
            finally:
                cleanup_caches()

    ...
    for i in range(2):
        for j in range(3):
            for k in range(4):
                ...
                new_val = y_np[k, i, j]
                assert original_val == new_val, f"Mismatch: x[{i},{j},{k}]={original_val} != y[{k},{i},{j}]={new_val}"
    ...
    for c in range(4):
        for d in range(5):
            for k in range(6):
                ...
                assert original_val == batched_val, f"Batch move failed: z[{c},{d},{k}]={original_val} != z_batched[{k},{c},{d}]={batched_val}"

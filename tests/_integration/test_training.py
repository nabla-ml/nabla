...
"""Integration test for MLP training functionality."""
...
TEST_BATCH_SIZE = 32
TEST_LAYERS = [1, 8, 8, 1]  # Smaller network for faster tests
...
def train_step_functional(
    ...
    learning_rate: float = TEST_LEARNING_RATE,
    momentum: float = TEST_MOMENTUM,
...
def test_mlp_training_with_benchmark():
    ...
    print(f"Network architecture: {TEST_LAYERS}")
    print(f"Learning rate: {TEST_LEARNING_RATE}")
    print(f"Momentum: {TEST_MOMENTUM}")
    print(f"Sin periods: {SIN_PERIODS}")
    print("Starting training...")

    # Initialize model parameters
    params = initialize_mlp_params(TEST_LAYERS)
    ...
    for epoch in range(1, TEST_NUM_EPOCHS + 1):
        start_time = time.perf_counter()

        # Create fresh batch each iteration (like Mojo)
        x, targets = create_sin_dataset(TEST_BATCH_SIZE)

        # Functional training step
        params, optimizer_state, loss = train_step_functional(
            x, targets, params, optimizer_state, TEST_LEARNING_RATE, TEST_MOMENTUM
        )

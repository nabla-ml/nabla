...
class TestTransformerPP(unittest.TestCase):
    def _create_weights(self, stages, d_model):
        ...
        return {
            k: np.random.randn(
                stages,
                d_model,
                d_model if k not in ["W1", "W2"] else (d_ff if k == "W1" else d_model),
            ).astype(np.float32)
    ...
    def _verify(self, mesh, x_shape, dp_axis=None):
        ...
        weights = {
            k: ops.shard(ops.constant(v), mesh, w_specs)
        ...
        for i in range(STAGES):
            out_numpy[(i + 1) % STAGES] = self._numpy_layer(
                x_data[i], {k: v[i] for k, v in weights_data.items()}, D
            )
    ...
    def test_pp_trace(self):
        ...
        x = ops.shard(
            ops.constant(np.zeros((2, 16, 32), np.float32)),
            mesh,
            [DimSpec(["stage"]), DimSpec([]), DimSpec([])],
        )
        weights = {
            k: ops.shard(
                ops.constant(v), mesh, [DimSpec(["stage"]), DimSpec([]), DimSpec([])]
            )
            for k, v in self._create_weights(2, 32).items()
        }

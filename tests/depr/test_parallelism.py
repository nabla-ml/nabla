...
def test_pipeline_parallelism():
    ...
    is_stage_0_mask_np = (
        (stage_ids_np == 0)
        .repeat(MICRO_BATCH_SIZE, axis=1)
        .repeat(DIM, axis=2)
        .astype(bool)
    )
    mask_sharded = ops.shard(
        nb.Tensor.from_dlpack(is_stage_0_mask_np), mesh, w_spec
    ).realize()
    ...
    init_state_sharded = ops.shard(
        nb.Tensor.from_dlpack(init_state_np), mesh, w_spec
    ).realize()
    ...
    def pipeline_forward_fn(all_inputs, weight_stack, stage_mask, state):
        ...
        for t in range(MICRO_BATCHES + STAGES):
            ...
            if t < MICRO_BATCHES:
                fresh_in = ops.gather(
                    all_inputs, ops.constant([t], dtype=DType.int64), axis=0
                )
    ...
    traced = nb.core.graph.tracing.trace(
        pipeline_forward_fn, x_nb, w_sharded, mask_sharded, init_state_sharded
    )
    print("\n=== TRACED GRAPH ===")
    print(traced)
    print("====================")

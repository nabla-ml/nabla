...
class TestPPMLPTraining(unittest.TestCase):
    def test_pp_grad_accumulation(self):
        ...
        weights_nb = {k: nb.Tensor.from_dlpack(v.copy()) for k, v in weights_np.items()}
        ...
        layer_batch_mapped = vmap(
            mlp_layer, in_axes=(0, None), out_axes=0, spmd_axis_name="data", mesh=mesh
        )
        ...
        def pp_train_step(x, weights):
            ...
            for t in range(1):
                ...
                # Loss accumulation
                accum_loss = accum_loss + ops.reduce_sum(
                    y, axis=list(range(len(y.shape)))
                )
        ...
        np.testing.assert_allclose(
            gx_nb, gx_jax, rtol=1e-4, atol=1e-5, err_msg="Input gradient mismatch"
        )
        ...
        for k in weights_np.keys():
            ...
            gjax = gw_jax[k]
            np.testing.assert_allclose(
                gnb, gjax, rtol=1e-4, atol=1e-5, err_msg=f"Weight grad mismatch for {k}"
            )

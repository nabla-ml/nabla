...
def test_streaming_pipeline_shift_register(dp_size):
    ...
    # Mesh Setup
    mesh = DeviceMesh(
        "pp_dp", (4, dp_size), ("stage", "data")
    )  # 4 Stages as in user example? User said STAGES=4.
    STAGES = 4
    ...
    x_stream_np = np.random.randn(MICRO_BATCHES, MICRO_BATCH_SIZE, DIM).astype(
        np.float32
    )
    targets_np = np.random.randn(MICRO_BATCHES, MICRO_BATCH_SIZE, DIM).astype(
        np.float32
    )
    ...
    layer_batched = vmap(
        dense_layer, in_axes=(0, None), out_axes=0, spmd_axis_name="data", mesh=mesh
    )

    # Kernel: Distributed Step (Compute + Shift)
    # Vmap over Stage dim
    # Inputs: x_state=(Stage, B, D), w_all=(Stage, D, D)
    layer_stage_batched = vmap(
        layer_batched, in_axes=(0, 0), out_axes=0, spmd_axis_name="stage", mesh=mesh
    )
    ...
    def pipeline_forward(x_stream, weights):
        ...
        # (STAGES, B, D)
        state = ops.zeros(
            (STAGES, MICRO_BATCH_SIZE, DIM),
            dtype=x_stream.dtype,
            device=mesh.devices[0],
        )
        state = ops.shard(
            state, mesh, [DimSpec(["stage"]), DimSpec(["data"]), DimSpec([])]
        )
    ...
    def loss_fn(x, w, t):
        # Pad inputs (flush)
        padding = ops.zeros(
            (STAGES - 1, MICRO_BATCH_SIZE, DIM), dtype=x.dtype, device=x.device
        )
    ...
    if HAS_JAX:
        ...
        np.testing.assert_allclose(
            g_w_nb, gw_ref, rtol=1e-4, atol=1e-4
        )  # Batched, loose tol

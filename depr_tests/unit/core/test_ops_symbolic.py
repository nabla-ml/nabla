...
def check_shape(tensor, expected_str):
    ...
    assert actual == expected_str, f"Expected {expected_str}, got {actual}"
...
def test_binary_operations():
    ...
    for name, result in tests.items():
        print(f"\n  {name}:")
        check_shape(result, "[Dim('b1'), Dim(64)]")
...
def test_operation_chain():
    ...
    y = h @ W2
    print(f"\n  y = h @ W2:")
    check_shape(y, "[Dim('b6'), Dim(10)]")


if __name__ == "__main__":
    # If run as script, execute tests manually

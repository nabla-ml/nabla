...
def positional_encoding(max_seq_len: int, d_model: int) -> jnp.ndarray:
    ...
    The encoding alternates between sine and cosine:
    - Even indices (0, 2, 4, ...): sin(pos / 10000^(2i/d_model))
    - Odd indices (1, 3, 5, ...):  cos(pos / 10000^(2i/d_model))
    ...
    position = jnp.arange(max_seq_len)[:, jnp.newaxis]  # Shape: (max_seq_len, 1)

    # Create dimension indices for each d_model position: [0, 1, 2, ..., d_model-1]
    dim_indices = jnp.arange(d_model)[jnp.newaxis, :]  # Shape: (1, d_model)

    # Calculate scaling factors: 10000^(2i/d_model) for each dimension
    scaling_factors = 10000.0 ** (2 * (dim_indices // 2) / d_model)

    # Calculate the angle for each position-dimension pair
    angles = position / scaling_factors  # Shape: (max_seq_len, d_model)

    # Apply sine to even indices, cosine to odd indices
    pe = jnp.where(
        dim_indices % 2 == 0,
        jnp.sin(angles),  # Even indices: sine
        jnp.cos(angles),  # Odd indices: cosine
    )

    # Add batch dimension
    return pe[jnp.newaxis, ...]  # Shape: (1, max_seq_len, d_model)
...
def encoder_layer(x, params, mask):
    ...
    Architecture: x -> LayerNorm(x + MultiHeadAttention(x)) -> LayerNorm(x + FFN(x))
    ...
    attention_output = multi_head_attention(x, x, params["mha"], mask)
    x_after_attention = layer_norm(x + attention_output, params["norm1"])

    # Feed-forward network with residual connection
    ffn_output = feed_forward(x_after_attention, params["ffn"])
    x_final = layer_norm(x_after_attention + ffn_output, params["norm2"])
...
def decoder_layer(x, encoder_output, params, look_ahead_mask, padding_mask):
    ...
    Architecture:
    1. x -> LayerNorm(x + MaskedMultiHeadAttention(x))
    2. x -> LayerNorm(x + CrossMultiHeadAttention(x, encoder_output))
    ...
    masked_attention_output = multi_head_attention(
        x, x, params["masked_mha"], look_ahead_mask
    )
    x_after_masked_attention = layer_norm(x + masked_attention_output, params["norm1"])

    # 2. Cross-attention between decoder and encoder
    cross_attention_output = multi_head_attention(
        x_after_masked_attention, encoder_output, params["cross_mha"], padding_mask
    )
    x_after_cross_attention = layer_norm(
        x_after_masked_attention + cross_attention_output, params["norm2"]
    )

    # 3. Feed-forward network
    ffn_output = feed_forward(x_after_cross_attention, params["ffn"])
    x_final = layer_norm(x_after_cross_attention + ffn_output, params["norm3"])
...
def transformer_forward(encoder_inputs, decoder_inputs, params):
    ...
    look_ahead_mask = (
        jnp.triu(jnp.ones((1, 1, target_seq_len, target_seq_len)), k=1) == 0
    )
...
def train_transformer():
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        if epoch % PRINT_INTERVAL == 0:
            ...
            print(f"Epoch {epoch:5d} | Loss: {loss:.4f} | Time: {elapsed:.1f}s")

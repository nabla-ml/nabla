...
def target_function_numpy(x):
    ...
    local_bump = 0.3 * np.exp(-1.5 * (x - 0.5)**2) * np.sin(8 * x)  # Less frantic oscillation
...
def generate_regression_data_numpy(num_samples=300, noise=0.0, seed=42):
    ...
    print(f"📊 Data range: [{x_data.min():.2f}, {x_data.max():.2f}] | Target range: [{y_data.min():.2f}, {y_data.max():.2f}]")
...
def init_network_params_nabla(layer_sizes, seed=42):
    ...
    for i, (in_size, out_size) in enumerate(zip(layer_sizes[:-1], layer_sizes[1:])):
        if i == len(layer_sizes) - 2:  # Output layer
            ...
            W_np = np.random.normal(0, np.sqrt(1.0 / in_size), (in_size, out_size)).astype(np.float32)
...
@nb.jit
def train_step_adam_nabla(params, m_state, v_state, x, y, lr, step, 
    ...
    for (W, b), (dW, db), (mW, mb), (vW, vb) in zip(params, grads, m_state, v_state):
...
def plot_function_approximation(params, x_data, y_data, x_grid, y_true_grid, layer_sizes, epoch=0, loss=0.0):
    ...
    ax_main.plot(x_grid, y_true_grid, color=color_true, linewidth=4, 
                label='Target Function', alpha=0.95, zorder=3)
    ax_main.plot(x_grid, y_pred_grid, color=color_pred, linewidth=3, 
                label='NN Prediction', alpha=0.9, zorder=4)
    ...
    ax_main.set_title(f'Neural Network Function Approximation\nArchitecture: {" → ".join(map(str, layer_sizes))} | Epoch {epoch:03d} | MSE: {loss:.6f}', 
                     fontsize=16, color=color_text, fontweight='bold', pad=25)
    ...
    if len(loss_history) > 1:
        ax_loss.plot(epoch_history, loss_history, color=color_loss, linewidth=3, alpha=0.9)
        ...
        ax_loss.set_title('Training Loss', fontsize=14, color=color_text, fontweight='bold', pad=20)
...
def main():
    ...
    x_grid, y_true_grid, x_bounds = create_evaluation_grid_numpy(x_data, resolution=1000, padding=0.5)
    ...
    initial_loss = mse_loss_nabla(params_container[0], x_data_nabla, y_data_nabla).to_numpy().item()
    plot_function_approximation(params_container[0], x_data, y_data, x_grid, y_true_grid, layer_sizes,
                              epoch=0, loss=initial_loss)
    
    def update(frame):
        current_lr = cosine_annealing_schedule_numpy(frame, learning_rate, num_epochs, min_lr_ratio=0.6)
        
        params_container[0], m_container[0], v_container[0], loss = train_step_adam_nabla(
            params_container[0], m_container[0], v_container[0], 
            x_data_nabla, y_data_nabla, current_lr, step_container[0]
        )
        step_container[0] += 1

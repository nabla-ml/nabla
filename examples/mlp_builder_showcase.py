...
import numpy as np
import nabla as nb
from nabla.nn import MLPBuilder, value_and_grad, create_sin_dataset
...
def train_with_builder():
    ...
    from nabla.nn import init_adamw_state, adamw_step, learning_rate_schedule
...
def demonstrate_different_configs():
    ...
    print(f"Config 1 output range: [{pred1.to_numpy().min():.3f}, {pred1.to_numpy().max():.3f}]")
    print(f"Config 2 output range: [{pred2.to_numpy().min():.3f}, {pred2.to_numpy().max():.3f}]")
...
def showcase_modular_components():
    ...
    from nabla.nn.init import he_normal, xavier_normal, lecun_normal
    ...
    print("\nLearning Rate Schedules:")
    for epoch in epochs:
        print(f"Epoch {epoch:4d}: Exp={exp_schedule(epoch):.6f}, "
...
if __name__ == "__main__":
    ...
    print(f"\n=== Summary ===")
    print(f"✅ Modular MLP training completed successfully!")
    print(f"✅ All components are properly organized in nabla.nn")
    print(f"✅ Builder pattern provides convenient configuration")
    print(f"✅ Individual components can be used independently")

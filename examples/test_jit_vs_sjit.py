...
def test_adamw_step(param, grad, m, v, step, lr=0.001, beta1=0.9, beta2=0.999, eps=1e-8):
...
def test_gradient_computation():
    ...
    inputs = [x, y]
    loss_jit, grads_jit = nb.value_and_grad(test_function_for_grad, argnums=[0, 1])(inputs)
    print(f"JIT value_and_grad - Loss: {loss_jit.to_numpy()}, Grads: {[g.to_numpy() for g in grads_jit]}")
    
    # Test value_and_grad with sjit
    sjit_grad_func = nb.sjit(lambda inputs: nb.value_and_grad(test_function_for_grad, argnums=[0, 1])(inputs))
    loss_sjit, grads_sjit = sjit_grad_func(inputs)
    print(f"SJIT value_and_grad - Loss: {loss_sjit.to_numpy()}, Grads: {[g.to_numpy() for g in grads_sjit]}")
...
def test_training_steps():
    ...
    for step in range(1, 6):
        param_jit, m_jit, v_jit, loss_jit = train_step_jit(x, y, param, m, v, step, 0.01)
    ...
    for step in range(1, 6):
        param_sjit, m_sjit, v_sjit, loss_sjit = train_step_sjit(x, y, param, m, v, step, 0.01)
        print(f"Step {step}: param={param_sjit.to_numpy()}, loss={loss_sjit.to_numpy()}")

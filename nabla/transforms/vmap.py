...
def _map_prefix(
    ...
    *extra_args: Any,
    ...
    if isinstance(tree, (list, tuple)) and isinstance(prefix, (list, tuple)):
        ...
        return type(tree)(
            [
                _map_prefix(fn, t, a, *extra_args)
                for t, a in zip(tree, prefix, strict=False)
            ]
        )
...
def _collect_from_prefix(
    ...
    def _collect(tree_part: Any, prefix_part: AxisSpec) -> None:
        ...
        elif isinstance(tree_part, (list, tuple)) and isinstance(
            prefix_part, (list, tuple)
        ):
            for t, a in zip(tree_part, prefix_part, strict=False):
                _collect(t, a)
...
def _get_batch_size(tensor: Tensor, axis: AxisSpec):
    ...
    if not isinstance(axis, int):
        raise TypeError(
            f"Expected int or None for axis at tensor leaf, got {type(axis).__name__}"
        )
...
def _validate_batch_sizes(
    ...
    if axis_size is not None:
        ...
        if first != expected:
            raise ValueError(
                f"axis_size={axis_size} doesn't match inferred batch dim {first}"
            )

    ...
    def jit_func(*args):
        ...
        if static:
            ...
            if cached_model is None:
                ...
                outputs = _clean_traced_outputs(
                    outputs, is_list_style, remove_staging=True
                )

            if not cached_model:
                raise RuntimeError(
                    "No cached model found. Note: A function which does not have any Arrays as inputs will fail to compile, since the tracing can not be properly initialized."
                )

            # Use the cached model for execution
            # Reorder inputs to match the model's expected order
            function_param_tensors = [
                input_node.impl for input_node in flat_input_arrays
            ]

...
def pullback(inputs: list[Array], outputs: list[Array], cotangents: list[Array], trace: Trace = None) -> list[Array]:
    """Compute vector-Jacobian product (reverse-mode autodiff)."""
    if len(cotangents) != len(outputs):
        raise ValueError(f"Cotangents length {len(cotangents)} != outputs length {len(outputs)}")
    ...
    try:
        ...
        for node in reversed(traced_nodes):
            ...
            if not hasattr(node, 'cotangent') or node.cotangent is None:
                continue
            ...
            try:
                ...
                for arg, arg_cotangent in zip(node.args, arg_cotangents):
                    if hasattr(arg, 'cotangent') and arg.cotangent is not None:
                ...
                if node not in inputs:
                    delattr(node, 'cotangent')
                        
            except Exception as e:
                raise RuntimeError(f"VJP rule failed for operation '{node.name}': {e}") from e
        
        # Step 5: Collect gradients for input nodes
        input_gradients = []
        for inp in inputs:
            if hasattr(inp, 'cotangent') and inp.cotangent is not None:
        ...
        for inp in inputs:
            if hasattr(inp, 'cotangent'):
                delattr(inp, 'cotangent')


def pushfwd(inputs: list[Array], outputs: list[Array], tangents: list[Array], trace: Trace = None) -> list[Array]:
    """Compute Jacobian-vector product (forward-mode autodiff)."""
    if len(tangents) != len(inputs):
        raise ValueError(f"Tangents length {len(tangents)} != inputs length {len(inputs)}")
    ...
    for input_node, tangent in zip(inputs, tangents):
    ...
    try:
        ...
        for node in traced_nodes:
            ...
            arg_tangents = []
            for arg in node.args:
                if hasattr(arg, 'tangent') and arg.tangent is not None:
        ...
        output_tangents = []
        for out in outputs:
            if hasattr(out, 'tangent') and out.tangent is not None:
        ...
        # Step 6: Cleanup - only remaining tangents (inputs, outputs, and any missed ones)
        for node in traced_nodes:
            if hasattr(node, 'tangent'):
                delattr(node, 'tangent')
...
class VJPTransform(Transformation):
    ...
    def post_call(self, outputs: list[Array]) -> tuple[list[Array], Callable]:
        ...
        def vjp_fn(cotangents: list[Array]) -> list[Array]:
            ...
            return pullback(self.trace.inputs, self.trace.outputs, cotangents, self.trace)
...
class JVPTransform(Transformation):
    ...
    def post_call(self, outputs: list[Array]) -> tuple[list[Array], list[Array]]:
        ...
        output_tangents = pushfwd(self.trace.inputs, self.trace.outputs, self.tangents, self.trace)
        
        return outputs, output_tangents


# High-level JAX-style API functions
def vjp(func: Callable, inputs: list[Array]) -> tuple[list[Array], Callable]:
    """
    ...
    return vjp_transform(inputs)

def jvp(func: Callable, inputs: list[Array], tangents: list[Array]) -> tuple[list[Array], list[Array]]:
    """

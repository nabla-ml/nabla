...
def sparse_cross_entropy_loss(logits: nb.Array, targets: nb.Array, axis: int = -1) -> nb.Array:
...
def binary_cross_entropy_loss(predictions: nb.Array, targets: nb.Array, eps: float = 1e-7) -> nb.Array:
    ...
    bce_per_sample = -(mul(targets, log_p) + mul(sub(nb.ones_like(targets), targets), log_one_minus_p))
    
    # Average over batch
    return mean(bce_per_sample)


def softmax_cross_entropy_loss(logits: nb.Array, targets: nb.Array, axis: int = -1) -> nb.Array:
    ...
    """
    from ...ops.special import logsumexp
    from ...ops.reduce import sum as array_sum, mean
    from ...ops.binary import mul

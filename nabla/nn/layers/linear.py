...
def linear_forward(x: nb.Array, weight: nb.Array, bias: nb.Array | None = None) -> nb.Array:
...
def mlp_forward_with_activations(
    ...
    from .activations import relu, leaky_relu, sigmoid, tanh, gelu, silu
    ...
    if activation not in activation_fns:
        raise ValueError(f"Unsupported activation: {activation}. Supported: {list(activation_fns.keys())}")

# Nabla Module: Lazy Eager Execution for MAX

## Overview

The `nabla` module implements a **Lazy Eager** execution framework that combines PyTorch's imperative API with JAX's compilation and transformation capabilities, targeting the MAX platform.
...
result = batched(nabla.Tensor.ones((10, 5)))  # Vectorized!
```

**Compile**:
```python
from nabla import compile

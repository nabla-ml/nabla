...
def generate_sitemap():
    ...
    if build_dir.exists():
        ...
        for html_file in html_files:
            ...
            if any(skip in str(rel_path) for skip in [
                '_static/', '_sources/', 'genindex.html', 'search.html',
                'py-modindex.html', 'searchindex.html'
            ]):
                continue
            ...
            # Determine priority
            priority = priorities.get(html_path, None)
            if priority is None:
            ...
            if html_path.startswith('api/'):
                changefreq = 'monthly'
            elif html_path.startswith('tutorials/'):
                changefreq = 'monthly'
    ...
    # Method 2: Fallback to scanning markdown sources if no build
        ...
        for md_file in md_files:
            ...
            if any(skip in str(rel_path) for skip in ['_build', '__pycache__', 'README']):
                continue
            ...
            changefreq = 'monthly' if ('api/' in html_path or 'tutorials/' in html_path) else 'weekly'

#!/usr/bin/env python
"""
Gradient correctness tests for nb.conv2d_transpose using imperative .backward(),
comparing against PyTorch autograd on the same inputs/weights/bias and dY.

This mirrors the configurations from tmp/conv/conv2d_transpose_manual_grad.py
but validates Nabla's gradients directly against PyTorch instead of a manual derivation.
"""
...
def _same_padding_for_transpose(kernel_shape, dilation, stride):
    # Only defined here for stride == 1
    ...
    if isinstance(stride, int):
        ...
        s_h, s_w = stride
    if s_h != 1 or s_w != 1:
        return None  # unsupported in our adaptation
    k_h, k_w = kernel_shape
    total_h = d_h * (k_h - 1)
    total_w = d_w * (k_w - 1)
    # For transposed conv we will use symmetric padding (top/left)
...
def verify_conv2d_transpose_grads_against_torch(test_config: dict) -> bool:
    name = test_config["name"]
    input_shape = test_config["input_shape"]  # (N, C_in, H, W)
    weight_shape = test_config["kernel_shape"]  # (C_in, C_out/groups, K_h, K_w) for transposed conv
    ...
    print(f"--- Conv2DTranspose Grads Test: {name} (dtype={dtype_name}) ---")
    _set_seed(42)

    N, C_in, H_in, W_in = input_shape
    C_in_w, C_out_per_g, K_h, K_w = weight_shape
    C_out = C_out_per_g * groups

    # Convert string paddings for transpose (not supported as strings in nb)
    pad_arg = padding
    if isinstance(padding, str):
        if padding == "valid":
            pad_arg = 0
        elif padding == "same":
            sym = _same_padding_for_transpose((K_h, K_w), dilation, stride)
            if sym is None:
    ...
    w_pt = torch.tensor(w_np, requires_grad=True, dtype=torch_dtype)
    b_pt = torch.tensor(b_np, requires_grad=True, dtype=torch_dtype) if use_bias else None

    try:
        y_pt = F.conv_transpose2d(
            x_pt, w_pt, bias=b_pt, stride=stride, padding=pad_arg,
    ...
    w_nb = nb.tensor(w_np).requires_grad_()
    b_nb = nb.tensor(b_np).requires_grad_() if use_bias else None

    try:
        y_nb = nb.conv2d_transpose(
            x_nb, w_nb, stride=stride, padding=pad_arg, output_padding=output_padding,
...
# Import test cases from the manual suite (matching conv2d_transpose_manual_grad.py)
conv2d_transpose_test_cases = [
    ...
    {"name": "Param: Stride>1", "input_shape": (2, 3, 10, 10), "kernel_shape": (3, 4, 3, 3), "stride": 3},
    {"name": "Param: Padding>0", "input_shape": (2, 3, 8, 8), "kernel_shape": (3, 4, 3, 3), "padding": 2},
    {"name": "Param: Dilation>1", "input_shape": (2, 3, 10, 10), "kernel_shape": (3, 4, 3, 3), "dilation": 3},
    # === 3. Parameter Combinations ===
    {"name": "Combo: Stride+Padding", "input_shape": (2, 3, 10, 10), "kernel_shape": (3, 4, 3, 3), "stride": 2, "padding": 1},
    {"name": "Combo: Stride+Dilation", "input_shape": (2, 3, 12, 12), "kernel_shape": (3, 4, 3, 3), "stride": 2, "dilation": 2},
    {"name": "Combo: Padding+Dilation", "input_shape": (2, 3, 12, 12), "kernel_shape": (3, 4, 3, 3), "padding": 2, "dilation": 2},
    # === 4. Shape-based Edge Cases ===
    {"name": "Shape: 1x1 Kernel", "input_shape": (2, 8, 10, 10), "kernel_shape": (8, 16, 1, 1), "stride": 1, "padding": 0},
    {"name": "Shape: Input==Kernel", "input_shape": (2, 3, 5, 5), "kernel_shape": (3, 4, 5, 5)},
    {"name": "Shape: Min Input for 1x1 Out", "input_shape": (2, 3, 7, 7), "kernel_shape": (3, 4, 3, 3), "dilation": 3},
    {"name": "Shape: Stride>Kernel", "input_shape": (1, 1, 10, 10), "kernel_shape": (1, 1, 3, 3), "stride": 4},
    # === 5. Grouped Convolution Cases ===
    {"name": "Groups: Standard", "input_shape": (2, 4, 10, 10), "kernel_shape": (4, 4, 3, 3), "groups": 2},
    {"name": "Groups: Depthwise", "input_shape": (2, 4, 10, 10), "kernel_shape": (4, 1, 3, 3), "groups": 4},
    {"name": "Groups: Depthwise Mult", "input_shape": (2, 4, 10, 10), "kernel_shape": (4, 2, 3, 3), "groups": 4},
    ...
    {"name": "Bias: Multi-channel", "input_shape": (2, 3, 8, 8), "kernel_shape": (3, 4, 3, 3), "bias": True},
    {"name": "Bias: Grouped", "input_shape": (2, 4, 10, 10), "kernel_shape": (4, 4, 3, 3), "groups": 2, "bias": True},
    {"name": "Bias: Depthwise", "input_shape": (2, 4, 10, 10), "kernel_shape": (4, 1, 3, 3), "groups": 4, "bias": True},
    {"name": "Bias: All Params", "input_shape": (2, 3, 12, 12), "kernel_shape": (3, 4, 3, 3), "stride": 2, "padding": 1, "dilation": 2, "bias": True},
    # === 9. Batch Size Variations ===
    {"name": "Batch: N=1 minimal", "input_shape": (1, 1, 3, 3), "kernel_shape": (1, 1, 3, 3)},
    {"name": "Batch: N=8", "input_shape": (8, 3, 10, 10), "kernel_shape": (3, 4, 3, 3)},
    {"name": "Batch: N=32 large", "input_shape": (32, 2, 6, 6), "kernel_shape": (2, 4, 3, 3)},
    # === 10. Channel Variations ===
    {"name": "Channel: C_in=1, C_out=1", "input_shape": (2, 1, 10, 10), "kernel_shape": (1, 1, 3, 3)},
    {"name": "Channel: C_in=1, C_out=many", "input_shape": (2, 1, 10, 10), "kernel_shape": (1, 32, 3, 3)},
    {"name": "Channel: C_in=many, C_out=1", "input_shape": (2, 32, 10, 10), "kernel_shape": (32, 1, 3, 3)},
    {"name": "Channel: High count", "input_shape": (2, 64, 10, 10), "kernel_shape": (64, 128, 3, 3)},
    # === 11. Minimal Spatial Dimensions ===
    {"name": "Spatial: 2x2 input", "input_shape": (1, 1, 2, 2), "kernel_shape": (1, 1, 3, 3), "padding": 1},
    {"name": "Spatial: 3x3 input, 3x3 kernel", "input_shape": (2, 3, 3, 3), "kernel_shape": (3, 4, 3, 3)},
    # === 12. Large Spatial Dimensions ===
    {"name": "Spatial: 64x64 input", "input_shape": (1, 3, 64, 64), "kernel_shape": (3, 4, 3, 3)},
    ...
    {"name": "Extreme: All params large asym", "input_shape": (2, 3, 40, 50), "kernel_shape": (3, 4, 5, 7), "stride": (3, 4), "padding": (2, 3), "dilation": (2, 2)},
    ...
    {"name": "Special Grad: Sparse", "input_shape": (2, 3, 8, 8), "kernel_shape": (3, 4, 3, 3), "special_grad": "sparse"},
    # === 17. Data Type Tests ===
    {"name": "DType: float32 basic", "input_shape": (1, 1, 5, 5), "kernel_shape": (1, 1, 3, 3), "dtype": "float32"},
    {"name": "DType: float32 grouped+bias", "input_shape": (2, 4, 10, 10), "kernel_shape": (4, 4, 3, 3), "groups": 2, "bias": True, "dtype": "float32"},
    {"name": "DType: float32 high channel", "input_shape": (2, 32, 10, 10), "kernel_shape": (32, 1, 3, 3), "dtype": "float32"},
    ...
    {"name": "STRESS: All params asym+groups", "input_shape": (4, 6, 20, 18), "kernel_shape": (6, 4, 5, 3), "stride": (2, 3), "padding": (3, 1), "dilation": (2, 1), "groups": 2},
    {"name": "STRESS: Everything+bias float32", "input_shape": (4, 6, 20, 18), "kernel_shape": (6, 4, 5, 3), "stride": (2, 3), "padding": (3, 1), "dilation": (2, 1), "groups": 2, "bias": True, "dtype": "float32"},
    {"name": "STRESS: Large batch+channels", "input_shape": (16, 64, 16, 16), "kernel_shape": (64, 128, 3, 3), "bias": True},
]


def run_all_conv2d_transpose_tests():
    if not TORCH_AVAILABLE:
        print("PyTorch not available - skipping all conv2d_transpose grad tests.")
        return True

    all_ok = True
    for cfg in conv2d_transpose_test_cases:
        ok = verify_conv2d_transpose_grads_against_torch(cfg)
        all_ok = all_ok and ok
    print("\nConv2D-Transpose grad tests:", "PASSED" if all_ok else "FAILED")
    return all_ok


if __name__ == "__main__":
    run_all_conv2d_transpose_tests()

...
def verify_conv3d_gradients(test_config: dict):
    """
    A testing function to verify the manually derived gradients for F.conv3d
    ...
    s_d, s_h, s_w = (stride, stride, stride) if isinstance(stride, int) else stride
    d_d, d_h, d_w = (dilation, dilation, dilation) if isinstance(dilation, int) else dilation

    if isinstance(padding, str):
        if padding == 'valid':
            pad_d0 = pad_d1 = pad_h0 = pad_h1 = pad_w0 = pad_w1 = 0
        elif padding == 'same':
            if s_d != 1 or s_h != 1 or s_w != 1:
                print(f"    ERROR: padding='same' is not supported for stride != 1")
                print("    --> Test SKIPPED.\n")
                return
            total_d = d_d * (kernel_shape[2] - 1)
            pad_d0 = total_d // 2; pad_d1 = total_d - pad_d0
            total_h = d_h * (kernel_shape[3] - 1)
            pad_h0 = total_h // 2; pad_h1 = total_h - pad_h0
            total_w = d_w * (kernel_shape[4] - 1)
        ...
        p_d, p_h, p_w = (padding, padding, padding) if isinstance(padding, int) else padding
        pad_d0 = pad_d1 = p_d; pad_h0 = pad_h1 = p_h; pad_w0 = pad_w1 = p_w

    # === 3. Setup Tensors ===
    (N, C_in, D_in, H_in, W_in) = input_shape
    (C_out, _, K_d, K_h, K_w) = kernel_shape
    ...
    # === 4. Forward Pass ===
    # Note: This implementation manually zero-pads and then uses conv3d(padding=0).
    # This means it *only* tests 'zero' padding mode.
    X_padded = F.pad(X, (pad_w0, pad_w1, pad_h0, pad_h1, pad_d0, pad_d1), mode='constant', value=0)
    try:
        Y = F.conv3d(X_padded, K, bias=B, stride=(s_d, s_h, s_w), padding=0, dilation=(d_d, d_h, d_w), groups=groups)
    except Exception as e:
        print(f"    ERROR during forward pass: {e}")
        print("    --> Test SKIPPED.\n")
        return
    (D_out, H_out, W_out) = Y.shape[2:]
    ...
    D_padded_in = D_in + pad_d0 + pad_d1
    H_padded_in = H_in + pad_h0 + pad_h1
    W_padded_in = W_in + pad_w0 + pad_w1
    D_prime = (D_out - 1) * s_d + d_d * (K_d - 1) + 1
    H_prime = (H_out - 1) * s_h + d_h * (K_h - 1) + 1
    W_prime = (W_out - 1) * s_w + d_w * (K_w - 1) + 1
    op_d_padded = D_padded_in - D_prime
    op_h_padded = H_padded_in - H_prime
    op_w_padded = W_padded_in - W_prime

    crop_d_end = max(0, -op_d_padded)
    crop_h_end = max(0, -op_h_padded)
    crop_w_end = max(0, -op_w_padded)
    op_d_padded = max(0, op_d_padded)
    op_h_padded = max(0, op_h_padded)
    op_w_padded = max(0, op_w_padded)

    manual_grad_X_padded = F.conv_transpose3d(
        grad_output, K, stride=(s_d, s_h, s_w), padding=0,
        output_padding=(op_d_padded, op_h_padded, op_w_padded),
        dilation=(d_d, d_h, d_w), groups=groups
    )

    if crop_d_end > 0: manual_grad_X_padded = manual_grad_X_padded[:, :, :-crop_d_end, :, :]
    ...
    d_end = -pad_d1 if pad_d1 > 0 else None
    h_end = -pad_h1 if pad_h1 > 0 else None
    w_end = -pad_w1 if pad_w1 > 0 else None
    manual_grad_X = manual_grad_X_padded[:, :, pad_d0:d_end, pad_h0:h_end, pad_w0:w_end]

    # 2. Gradient with respect to the Kernel (∂L/∂K)
    k_grad_method = "conv3d (efficient)" if groups == 1 else "conv3d (grouped)"

    C_in_g = C_in // groups
    C_out_g = C_out // groups

    D_pad_eff = d_d * (K_d - 1) + s_d * (D_out - 1) + 1
    H_pad_eff = d_h * (K_h - 1) + s_h * (H_out - 1) + 1
    W_pad_eff = d_w * (K_w - 1) + s_w * (W_out - 1) + 1

    manual_grad_K_list = []
    for g in range(groups):
        X_g = X_padded[:, g * C_in_g : (g + 1) * C_in_g, :, :, :]
        grad_out_g = grad_output[:, g * C_out_g : (g + 1) * C_out_g, :, :, :]
        X_g_cropped = X_g[:, :, :D_pad_eff, :H_pad_eff, :W_pad_eff]
        x_permuted = X_g_cropped.permute(1, 0, 2, 3, 4)  # C_in_g, N, D_pad_eff, H_pad_eff, W_pad_eff
        grad_output_permuted = grad_out_g.permute(1, 0, 2, 3, 4)  # C_out_g, N, D_out, H_out, W_out
        grad_K_permuted = F.conv3d(
            input=x_permuted, weight=grad_output_permuted,
            stride=(d_d, d_h, d_w), dilation=(s_d, s_h, s_w), padding=0, groups=1
        )
        grad_K_g = grad_K_permuted.permute(1, 0, 2, 3, 4)  # C_out_g, C_in_g, K_d, K_h, K_w
        manual_grad_K_list.append(grad_K_g)
    manual_grad_K = torch.cat(manual_grad_K_list, dim=0)

    # 3. Gradient with respect to Bias (∂L/∂B)
    manual_grad_B = grad_output.sum(dim=(0, 2, 3, 4)) if B is not None else None
...
# and then uses conv3d(padding=0). The manual ∂L/∂X calculation
# (using conv_transpose3d and cropping) is the mathematical
...
test_cases = [
    # === 1. Basic Sanity Checks ===
    {
        ...
        "input_shape": (1, 1, 5, 5, 5), "kernel_shape": (1, 1, 3, 3, 3),
    },
    {
    ...
    # === 2. Parameter Isolation ===
    {
        ...
        "input_shape": (2, 3, 10, 10, 10), "kernel_shape": (4, 3, 3, 3, 3),
        "stride": 3,
    },
    {
        "name": "Param Isolation: Padding > 0",
        "input_shape": (2, 3, 8, 8, 8), "kernel_shape": (4, 3, 3, 3, 3),
        "padding": 2,
    },
    {
    ...
    },
    {
        ...
        "input_shape": (2, 3, 12, 12, 12), "kernel_shape": (4, 3, 3, 3, 3),
        "stride": 2, "dilation": 2,
    },
    {
    ...
    {
        "name": "Shape Edge Case: 1x1x1 Kernel (Pointwise Conv)",
        "input_shape": (2, 8, 10, 10, 10), "kernel_shape": (16, 8, 1, 1, 1),
        "stride": 1, "padding": 0,
    },
    {
        "name": "Shape Edge Case: Input size = Kernel size",
        "input_shape": (2, 3, 5, 5, 5), "kernel_shape": (4, 3, 5, 5, 5),
    },
    {
        "name": "Shape Edge Case: Minimal Input for 1x1x1 Output (Dilation)",
        "input_shape": (2, 3, 7, 7, 7), "kernel_shape": (4, 3, 3, 3, 3),
        "dilation": 3,  # Dilated kernel is 1 + (3-1)*3 = 7x7x7
    },
    {
        "name": "Shape Edge Case: Stride > Kernel Size",
        "input_shape": (1, 1, 10, 10, 10), "kernel_shape": (1, 1, 3, 3, 3),
        "stride": 4,
    },
    # === 5. Grouped Convolution Cases (Tests both k_grad methods) ===
    {
        "name": "Groups: Standard Grouped Convolution",
        "input_shape": (2, 4, 10, 10, 10), "kernel_shape": (8, 2, 3, 3, 3),
        "groups": 2,
    },
    {
        "name": "Groups: Depthwise Convolution (C_in=C_out=groups)",
        "input_shape": (2, 4, 10, 10, 10), "kernel_shape": (4, 1, 3, 3, 3),
        "groups": 4,
    },
    {
        "name": "Groups: Depthwise Multiplier (C_out = K * C_in)",
        "input_shape": (2, 4, 10, 10, 10), "kernel_shape": (8, 1, 3, 3, 3),
        "groups": 4,  # C_out (8) is 2 * C_in (4)
    },
    # === 6. Asymmetric (Tuple) Parameter Tests ===
    {
        "name": "Asymmetric: Stride and Kernel",
        "input_shape": (1, 1, 10, 10, 12), "kernel_shape": (1, 1, 3, 3, 5),
        "stride": (2, 2, 1),
    },
    {
        "name": "Asymmetric: Padding and Dilation",
        "input_shape": (1, 1, 10, 10, 10), "kernel_shape": (1, 1, 3, 3, 3),
        "padding": (2, 1, 1), "dilation": (1, 1, 2),
    },
    # === 7. String Padding Tests ===
    {
        "name": "String Padding: 'valid'",
        "input_shape": (1, 1, 8, 8, 8), "kernel_shape": (1, 1, 3, 3, 3),
        "padding": "valid",
    },
    {
    ...
    },
    {
        ...
        "input_shape": (1, 1, 8, 8, 8), "kernel_shape": (1, 1, 4, 4, 4),
        "padding": "same", "stride": 1,
    },
    {
    ...
    # === 9. Batch Size Variations ===
    {
        ...
        "input_shape": (1, 1, 3, 3, 3), "kernel_shape": (1, 1, 3, 3, 3),
    },
    {
        "name": "Batch Size: N=8",
        "input_shape": (8, 3, 10, 10, 10), "kernel_shape": (4, 3, 3, 3, 3),
    },
    {
        "name": "Batch Size: N=32 very large",
        "input_shape": (32, 2, 6, 6, 6), "kernel_shape": (4, 2, 3, 3, 3),
    },
    # === 10. Channel Variations ===
    {
        "name": "Channel: C_in=1, C_out=1",
        "input_shape": (2, 1, 10, 10, 10), "kernel_shape": (1, 1, 3, 3, 3),
    },
    {
        "name": "Channel: C_in=1, C_out=many",
        "input_shape": (2, 1, 10, 10, 10), "kernel_shape": (32, 1, 3, 3, 3),
    },
    {
        "name": "Channel: C_in=many, C_out=1",
        "input_shape": (2, 32, 10, 10, 10), "kernel_shape": (1, 32, 3, 3, 3),
    },
    {
        "name": "Channel: High channel count",
        "input_shape": (2, 64, 10, 10, 10), "kernel_shape": (128, 64, 3, 3, 3),
    },
    # === 11. Minimal Spatial Dimensions ===
    {
        "name": "Spatial Edge: 2x2x2 input",
        "input_shape": (1, 1, 2, 2, 2), "kernel_shape": (1, 1, 3, 3, 3),
        "padding": 1,
    },
    {
        "name": "Spatial Edge: 3x3x3 input with 3x3x3 kernel",
        "input_shape": (2, 3, 3, 3, 3), "kernel_shape": (4, 3, 3, 3, 3),
    },
    # === 12. Large Spatial Dimensions ===
    {
        "name": "Spatial Large: 64x64x64 input",
        "input_shape": (1, 3, 64, 64, 64), "kernel_shape": (4, 3, 3, 3, 3),
    },
    # === 13. Non-Square Kernels and Inputs ===
    {
        "name": "Non-square: Rectangular kernel 3x3x5",
        "input_shape": (1, 1, 10, 10, 12), "kernel_shape": (1, 1, 3, 3, 5),
    },
    {
        "name": "Non-square: Asymmetric stride + kernel",
        "input_shape": (1, 1, 15, 15, 20), "kernel_shape": (1, 1, 3, 3, 5),
        "stride": (2, 2, 3), "padding": (1, 1, 2),
    },
    # === 14. Extreme Parameter Combinations ===
    {
        "name": "Extreme: Very large stride",
        "input_shape": (1, 1, 20, 20, 20), "kernel_shape": (1, 1, 3, 3, 3),
        "stride": 7,
    },
    {
        "name": "Extreme: Very large dilation",
        "input_shape": (1, 1, 30, 30, 30), "kernel_shape": (1, 1, 3, 3, 3),
        "dilation": 5,
    },
    {
        "name": "Extreme: All params large asymmetric",
        "input_shape": (2, 3, 40, 40, 50), "kernel_shape": (4, 3, 5, 5, 7),
        "stride": (3, 3, 4), "padding": (2, 2, 3), "dilation": (2, 2, 2),
    },
    # === 15. One-Dimensional Output Cases ===
    {
        "name": "Output 1x1x1: Heavy stride on small input",
        "input_shape": (1, 1, 10, 10, 10), "kernel_shape": (1, 1, 3, 3, 3),
        "stride": 9,
    },
    {
        "name": "Output 1x1x1: via dilation + kernel size",
        "input_shape": (1, 1, 7, 7, 7), "kernel_shape": (1, 1, 3, 3, 3),
        "dilation": 3,
    },
    ...
    },
    {
        ...
        "input_shape": (2, 4, 8, 8, 8), "kernel_shape": (4, 1, 3, 3, 3),
    ...
    # === 19. KITCHEN SINK STRESS TESTS ===
    {
        ...
        "input_shape": (4, 6, 20, 20, 18),
        "kernel_shape": (8, 3, 5, 5, 3),  # C_in_per_group = 6/2=3
        "stride": (2, 2, 3),
        "padding": (3, 3, 1),
        "dilation": (2, 2, 1),
        "groups": 2,
    ...
    },
    {
        ...
        "input_shape": (16, 64, 16, 16, 16),
        "kernel_shape": (128, 64, 3, 3, 3),
        "bias": True,
    },
]
...
if __name__ == '__main__':
    ...
    for config in test_cases:
        verify_conv3d_gradients(config)

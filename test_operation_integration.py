...
def test_automatic_device_movement():
    ...
    try:
        ...
        print(
            f"Input devices: CPU({cpu_arr.size} elements), GPU({gpu_arr.size} elements)"
        )
        ...
        print("Expected: GPU (accelerator preferred despite less data)")
        ...
        print(
            f"Input devices: CPU({cpu_mat1.size} elements), GPU({gpu_mat2.size} elements)"
        )

        result = cpu_mat1 @ gpu_mat2  # Uses matmul
        print(f"Result device: {result.device.label}")
        print("Expected: GPU (accelerator preferred)")

        # Test 3: Concatenation
        print("\n--- Test 3: Concatenation ---")
        cpu_arr1 = nb.array(
            [[1, 2, 3, 4, 5]] * 10, device=cpu_device
        )  # 50 elements on CPU
        gpu_arr1 = nb.array([[6, 7]], device=gpu_device)  # 2 elements on GPU

        print(
            f"Input devices: CPU({cpu_arr1.size} elements), GPU({gpu_arr1.size} elements)"
        )
        ...
        print("Input devices: Both on CPU")

        result = arr1[0] + arr2  # Should both stay on CPU
        print(f"Result device: {result.device.label}")
        print("Expected: CPU (only available device)")

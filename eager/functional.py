...
def functional(op: Op) -> Op:
    ...
    This decorator wraps MAX graph operations to work with the eager Tensor API.
    It handles:
    - Executing the op within the appropriate graph context
    - Converting results to Tensor objects  
    - Setting up TensorImpl with parent references for autograd tracing
    """
    from . import tensor  # Lazy import to avoid circular dependency
    from .tensor import TensorImpl

    def get_parent_impls(args, kwargs) -> tuple[list[TensorImpl], bool]:
        """Extract parent TensorImpls and determine if any are traced.
        
        Returns:
            Tuple of (parent_impls, any_traced)
        """
        ...
        for arg in args:
            if isinstance(arg, tensor.Tensor):
                ...
                # Handle lists of tensors (e.g., for concat, stack)
                for item in arg:
                    if isinstance(item, tensor.Tensor):
                        parent_impls.append(item._impl)
                        if item._impl.traced:
                            any_traced = True
        
        for v in kwargs.values():
            if isinstance(v, tensor.Tensor):
                parent_impls.append(v._impl)
                if v._impl.traced:
                    any_traced = True
                    
        return parent_impls, any_traced

    def to_tensor_with_impl(
        result: driver.Tensor | tensor.Tensor | TensorValue | BufferValue,
        parent_impls: list[TensorImpl],
        traced: bool,
        op_ref: Op,
    ) -> tensor.Tensor:
        """Convert a result to Tensor with proper TensorImpl setup."""
        if isinstance(result, tensor.Tensor):
            # Already a tensor, update its impl with parent info if needed
            if traced and not result._impl.parents:
                result._impl.parents = list(parent_impls)
                result._impl.op = op_ref
                result._impl.traced = True
            return result
        
        if isinstance(result, driver.Tensor):
            # Realized tensor from driver
            impl = TensorImpl(
                storage=result,
            ...
            return tensor.Tensor(impl=impl)
        
        # Graph value (TensorValue or BufferValue)
        impl = TensorImpl(
            value=result,
            parents=parent_impls,
            op=op_ref,
    ...
    def wrapped(*args, **kwargs):  # noqa: ANN202
        ...
        # Get parent impls and determine if tracing is active
        parent_impls, any_traced = get_parent_impls(args, kwargs)
        
        if isinstance(
            results, driver.Tensor | tensor.Tensor | TensorValue | BufferValue
        ):
            # Single result
            return to_tensor_with_impl(results, parent_impls, any_traced, op)
        
        # Multiple results
        return [
            to_tensor_with_impl(result, parent_impls, any_traced, op)

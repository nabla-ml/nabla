...
from max import _core, driver, engine, graph, mlir
...
class TensorImpl:
    ...
    """
    __slots__ = ('_values', '_storages', 'sharding', 'parents', 'op', 'traced', 'grad', 'batch_dims')
    
    _values: list[graph.BufferValue | graph.TensorValue]  # Per-shard graph values
    _storages: list[driver.Tensor] | None  # Per-shard realized data, or None
    sharding: object | None  # ShardingSpec when implemented, None = unsharded
    parents: list[TensorImpl]
    op: Op | None  # Will be Operation type, but keeping Op for now
    traced: bool
    grad: TensorImpl | None
    batch_dims: int  # Number of batch dimensions (prefix of physical shape)
    ...
    def __init__(
        ...
        # Store as lists for sharding-readiness (currently always single-element)
        self._values = [value] if value is not None else []
        self._storages = [storage] if storage is not None else None
        self.sharding = sharding
        # Only store parents if tracing is enabled
    ...
    def op_name(self) -> str | None:
        ...
        if self.op is None:
            return None
        # Handle both Operation objects and callables
    ...
    def physical_shape(self) -> tuple[int, ...] | None:
        """Actual storage shape (including batch dims at prefix).
        
        For sharded tensors, returns the local shard shape.
        """
        if self._storages is not None and len(self._storages) > 0:
            return tuple(self._storages[0].shape)
        if self._values and len(self._values) > 0:
            return tuple(self._values[0].type.shape)
        return None
    
    @property
    def logical_shape(self) -> tuple[int, ...] | None:
        """Shape as user sees it (excluding batch dims prefix)."""
        physical = self.physical_shape
        if physical is None:
            return None
        # Batch dims are always the prefix, so just slice them off
...
def get_topological_order(impl: TensorImpl) -> list[TensorImpl]:
    ...
    Returns a list of TensorImpls in the order they should be processed
    during backward pass (leaves first, output last).
    
    Args:
        impl: The output TensorImpl to start traversal from.
        
    Returns:
        List of TensorImpls in topological order.
    """
...
def print_computation_graph(impl: TensorImpl) -> None:
    ...
    Args:
        impl: The output TensorImpl to print the graph for.
    """
...
class Tensor(DLPackArray, HasTensorValue):
    ...
    **Interoperability:**
    ...
    #: The implementation object containing all tensor internals.
    _impl: TensorImpl
    #: Whether this tensor has been realized (computed).
    _real: bool = False
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            storage: Realized tensor data (driver.Tensor).
            value: Graph value for lazy execution.
            impl: Existing TensorImpl to wrap (for internal use).
            traced: Whether to enable tracing for autograd.
        """
        if impl is not None:
            # Use provided impl directly
            self._impl = impl
        else:
            # Create new TensorImpl
            assert storage is not None or value is not None
            self._impl = TensorImpl(storage=storage, value=value, traced=traced)
    ...
    def _storages(self) -> list[driver.Tensor] | None:
        """Per-shard storage (list of driver.Tensors, or None if unrealized)."""
    ...
    def _values(self) -> list:
        """Per-shard graph values."""
        return self._impl._values
    
    @_values.setter
    def _values(self, value: list) -> None:
        self._impl._values = value
    
    # ===== Convenience single-shard access (for unsharded tensors) =====
    
    @property
    def storage(self) -> driver.Tensor | None:
        """First shard's storage (convenience for unsharded tensors)."""
    ...
    def _value(self) -> graph.BufferValue | graph.TensorValue | None:
        """First shard's graph value (convenience for unsharded tensors)."""
    ...
    def _backing_value(
        ...
        """The backing value for shape/dtype/device queries."""
    ...
    def traced(self) -> bool:
        """Whether this tensor is being traced for autograd."""
    ...
    def trace(self) -> Tensor:
        ...
        Example:
            x = Tensor.ones((2, 2)).trace()
            y = x + 1  # y will also be traced
        """
